...
def add_common_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "--runs-dir", default="runs", help="Carpeta donde se guardan runs"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-ollama",
        action="store_true",
        help="Desactiva Ollama y usa fallback por plantilla",
    )
    parser.add_argument(
        "--hypothalamus-model", default=None, help="Modelo Ollama para Hipotálamo"
    )
    parser.add_argument(
        "--central-model", default=None, help="Modelo Ollama para Central"
    )
...
def load_run_learning_candidate(
    ...
    run_id = str(
        output_payload.get("run_id") or input_payload.get("run_id") or run_path.name
    )
    user_input = str(input_payload.get("user_input") or "").strip()
    response = str(output_payload.get("response") or "").strip()
    if not user_input or not response:
        raise ValueError(
            f"El run {run_id} no contiene input/output suficiente para aprendizaje."
        )

    actions = output_payload.get("actions_taken") or []
    report_status = (
        report_payload.get("status") if isinstance(report_payload, dict) else None
    )
...
def _read_run_json(
    run_path: Path, name: str, required: bool = True
) -> dict[str, object]:
    path = run_path / name
    if not path.exists():
        if required:
...
def handle_models(args: argparse.Namespace) -> None:
    ...
    if args.models_command == "cognitive-policy":
        ...
        reasoning_ready = bool(
            cognitive_health.get("ok")
            and cognitive_health.get("reasoning_model_available")
        )
        embedding_ready = bool(
            cognitive_health.get("ok")
            and cognitive_health.get("embedding_model_available")
        )
        ...
        print_json(
            {
                ...
                "policies": {
                    role: get_model_cognitive_policy(
                        role=role,
                        ollama_available=embedding_ready
                        if role == "semantic_embedding"
                        else reasoning_ready,
                        requested_model=selected.get(
                            "embeddings"
                            if role == "semantic_embedding"
                            else "reasoning"
                        ),
        ...
        print_json(
            {
                "status": "ok",
                "ollama": health,
                "router": router.route_many(intent=args.intent, urgency=args.urgency),
            }
        )
        return
...
def handle_learn(args: argparse.Namespace) -> None:
    ...
    if args.learn_command == "list":
        print_json(
            {
                "status": "ok",
                "candidates": pipe.list_candidates(
...
def handle_semantic_continuity(args: argparse.Namespace) -> None:
    continuity = SemanticContinuity(
        db_path=args.db, auto_ollama_embed=not args.no_ollama_embed
    )
...
def handle_federate(args: argparse.Namespace) -> None:
    ...
    if args.federate_command == "capabilities":
        print_json(
            federation.update_capabilities(args.node_id, json.loads(args.payload))
        )
        return
    ...
    if args.federate_command == "capable":
        print_json(
            {
                "status": "ok",
                "nodes": federation.list_capable_nodes(
                    min_tier=args.min_tier, require_gpu=args.require_gpu
                ),
            }
        )
        return
    ...
    if args.federate_command == "exchanges":
        print_json(
            {
                "status": "ok",
                "exchanges": federation.list_exchanges(
                    node_id=args.node_id, limit=args.limit
                ),
            }
        )
        return
...
def handle_qualia(args: argparse.Namespace) -> None:
    ...
    if args.qualia_command == "state":
        print_json(
            {
                "status": "ok",
                "state": store.latest_state(run_id=args.run_id),
                "states": store.list_states(run_id=args.run_id, limit=args.limit),
            }
        )
        return
    ...
    if args.qualia_command == "publish-test":
        ...
        print_json(
            bus.publish_experience(exp, ingest_learning=bool(args.proposed_learning))
        )
        return
...
def handle_workers(args: argparse.Namespace) -> None:
    ...
    if args.workers_command == "once":
        print_json(
            service.run_once(dry_run=args.dry_run, task_timeout=args.task_timeout)
        )
        return
...
def handle_runtime(args: argparse.Namespace) -> None:
    ...
    if args.runtime_command == "stop":
        print_json(
            stop_internal_runtime_background(db_path=args.db, runs_dir=args.runs_dir)
        )
        return
    if args.runtime_command == "events":
        print_json(
            {
                "status": "ok",
                "count": len(list_recent_events(limit=args.limit, db_path=args.db)),
                "events": list_recent_events(limit=args.limit, db_path=args.db),
            }
        )
        return
    if args.runtime_command == "context":
        print_json(
            build_living_context_for_chat(
...
def handle_relay(args: argparse.Namespace) -> None:
    ...
    headers = (
        {"Authorization": f"Bearer {args.admin_token}"} if args.admin_token else {}
    )
    ...
    if args.relay_command == "benchmark":
        ...
        client = PublicRelayClient(base_url=base_url, admin_token=args.admin_token)
        print_json(
            client.benchmark_online_nodes(
                federation, seconds=args.seconds, wait_timeout=args.wait_timeout
            )
        )
        return
                ...
                "nodes": [
                    {
                        ...
                        "benchmark_score": (node.get("capabilities") or {}).get(
                            "benchmark_score", 0
                        ),
                        "model_support": (node.get("capabilities") or {}).get(
                            "model_support", {}
                        ),
                    }
                    for node in nodes
...
def handle_always_on(args: argparse.Namespace) -> None:
    ...
    if args.always_on_command == "enable":
        ...
        print_json(
            {
                "status": "ok",
                "message": "ALWAYS-ON habilitado en triade.yml.",
                "config": yml["runtime"],
            }
        )
        return
    ...
    if args.always_on_command == "disable":
        ...
        print_json(
            {
                "status": "ok",
                "message": "ALWAYS-ON deshabilitado en triade.yml.",
                "stop_result": result,
            }
        )
        return
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Tríade Ω · MVP local auditable con memoria SQLite"
    )
    subparsers = parser.add_subparsers(dest="command")

    run_parser = subparsers.add_parser(
        "run", help="Ejecuta un mensaje como run auditable"
    )
    ...
    recall_parser = subparsers.add_parser(
        "recall", help="Consulta memoria episódica reciente"
    )
    recall_parser.add_argument(
        "query", nargs="?", default="", help="Texto a buscar en memoria"
    )
    recall_parser.add_argument(
        "--limit", type=int, default=10, help="Cantidad máxima de episodios"
    )
    recall_parser.add_argument(
        "--db", default="triade/memory/triade.db", help="Ruta de base SQLite"
    )
    ...
    analyze_parser = subparsers.add_parser(
        ...
        help="Analiza conversaciones locales en modo solo lectura",
    )
    add_analyze_conversations_args(analyze_parser)

    reflect_parser = subparsers.add_parser(
        "reflect-core", help="Reflexiona sobre el nucleo y propone mejoras/neuronas"
    )
    add_reflect_core_args(reflect_parser)

    semantic_continuity_parser = subparsers.add_parser(
        "semantic-continuity", help="Gestiona continuidad semántica real desde runs"
    )
    semantic_continuity_parser.add_argument(
        "--db", default="triade/memory/triade.db", help="Ruta de base SQLite"
    )
    semantic_continuity_parser.add_argument(
        "--no-ollama-embed", action="store_true", help="Usa solo embedding local hash"
    )
    semantic_continuity_sub = semantic_continuity_parser.add_subparsers(
        dest="semantic_continuity_command"
    )
    semantic_continuity_sub.add_parser(
        "doctor", help="Diagnostica documentos/embeddings de continuidad"
    )
    semantic_backfill = semantic_continuity_sub.add_parser(
        "backfill-runs",
        help="Crea documentos semánticos candidatos desde runs recientes",
    )
    semantic_backfill.add_argument(
        "--limit", type=int, default=50, help="Cantidad de runs recientes"
    )
    ...
    align_parser.add_argument(
        ...
        nargs="*",
        default=None,
        help="Lista opcional de artefactos de un run",
    )
    ...
    api_parser.add_argument(
        "--reload", action="store_true", help="Recarga automática para desarrollo"
    )

    models_parser = subparsers.add_parser(
        "models", help="Recomienda modelos por rol/tarea"
    )
    models_parser.add_argument(
        "--ollama-url", default="http://127.0.0.1:11434", help="URL local de Ollama"
    )
    models_subparsers = models_parser.add_subparsers(dest="models_command")

    models_route = models_subparsers.add_parser(
        "route", help="Recomienda un modelo para un rol"
    )
    models_route.add_argument(
        "--role",
        default="central",
        help="Rol: hypothalamus, central, creator, trainer, coder, embedding, fast, deep",
    )
    models_route.add_argument(
        "--intent", default="conversation", help="Intención detectada"
    )
    models_route.add_argument(
        "--urgency", default="medium", help="Urgencia: low, medium, high"
    )
    models_route.add_argument(
        "--prefer-speed", action="store_true", help="Prioriza velocidad"
    )
    models_route.add_argument(
        "--prefer-depth", action="store_true", help="Prioriza profundidad"
    )

    models_doctor = models_subparsers.add_parser(
        "doctor", help="Muestra recomendaciones para todos los roles"
    )
    models_doctor.add_argument(
        "--intent", default="conversation", help="Intención detectada"
    )
    models_doctor.add_argument(
        "--urgency", default="medium", help="Urgencia: low, medium, high"
    ...
    models_subparsers.add_parser(
        "ollama-health", help="Diagnostica Ollama como motor cognitivo local"
    )
    models_subparsers.add_parser(
        "ollama-blood", help="Diagnostica Ollama Blood como sangre cognitiva local"
    )
    models_subparsers.add_parser(
        "cognitive-policy", help="Muestra la política cognitiva por rol"
    )

    learn_parser = subparsers.add_parser(
        "learn", help="Pipeline de aprendizaje controlado (learning_queue)"
    )
    ...
    learn_ingest = learn_subparsers.add_parser(
        "ingest", help="Ingesta un candidato de aprendizaje"
    )
    learn_ingest.add_argument("content", help="Contenido del candidato")
    learn_ingest.add_argument(
        "--source-type",
        dest="source_type",
    ...
    learn_ingest.add_argument(
        "--source-ref", dest="source_ref", default=None, help="Referencia de fuente"
    )
    learn_ingest.add_argument("--title", default=None, help="Título del candidato")
    learn_ingest.add_argument(
        "--domain", default="general", help="Dominio del candidato"
    )
    learn_ingest.add_argument("--risk", default="low", help="low|medium|high|critical")

    learn_from_run = learn_subparsers.add_parser(
        "from-run", help="Crea candidato desde un run auditable"
    )
    learn_from_run.add_argument("run", help="ID del run o ruta a carpeta de run")
    learn_from_run.add_argument(
        "--runs-dir", default="runs", help="Carpeta base de runs"
    )
    learn_from_run.add_argument("--title", default=None, help="Título del candidato")
    learn_from_run.add_argument(
        "--domain", default="triade-runs", help="Dominio del candidato"
    )
    learn_from_run.add_argument(
        "--risk", default=None, help="Override de riesgo: low|medium|high|critical"
    )

    learn_eval = learn_subparsers.add_parser(
        "evaluate", help="Evalúa utilidad, confianza y riesgo"
    )
    learn_eval.add_argument("candidate_id", help="ID del candidato")

    learn_verify = learn_subparsers.add_parser(
        "verify", help="Verifica un candidato evaluado"
    )
    learn_verify.add_argument("candidate_id", help="ID del candidato")

    learn_consol = learn_subparsers.add_parser(
        "consolidate", help="Consolida a memoria estable (autónomo)"
    )
    learn_consol.add_argument("candidate_id", help="ID del candidato")
    learn_consol.add_argument(
        "--approved-by", dest="approved_by", required=True, help="Aprobador humano"
    )
    ...
    learn_list = learn_subparsers.add_parser(
        "list", help="Lista candidatos de aprendizaje"
    )
    ...
    learn_subparsers.add_parser(
        "doctor", help="Diagnóstico del pipeline de aprendizaje"
    )

    fed_parser = subparsers.add_parser(
        "federate", help="Federación entre nodos autorizados"
    )
    ...
    fed_register = fed_subparsers.add_parser(
        "register", help="Registra o actualiza un nodo federado"
    )
    ...
    fed_register.add_argument(
        "--permission", action="append", help="Permiso autorizado; se puede repetir"
    )
    fed_register.add_argument(
        "--capabilities", default=None, help="Perfil hardware JSON opcional del nodo"
    )
    ...
    fed_capabilities = fed_subparsers.add_parser(
        "capabilities", help="Actualiza capacidades hardware de un nodo"
    )
    fed_capabilities.add_argument("node_id", help="Identificador del nodo")
    fed_capabilities.add_argument(
        "--payload", required=True, help="Perfil hardware/capacidades en JSON"
    )

    fed_local_capabilities = fed_subparsers.add_parser(
        "detect-capabilities", help="Detecta y guarda capacidades locales para un nodo"
    )
    fed_local_capabilities.add_argument("node_id", help="Identificador del nodo")

    fed_capable = fed_subparsers.add_parser(
        "capable", help="Lista nodos activos aptos para computo"
    )
    fed_capable.add_argument("--min-tier", default="low", help="low|medium|high")
    fed_capable.add_argument(
        "--require-gpu", action="store_true", help="Exige GPU/VRAM detectada"
    )

    fed_receive = fed_subparsers.add_parser(
        "receive", help="Recibe un intercambio de un nodo"
    )
    fed_receive.add_argument("node_id", help="Nodo origen")
    fed_receive.add_argument(
        "--type",
        required=True,
        help="knowledge|pattern|neuron_spec|verification|learning_candidate",
    )
    fed_receive.add_argument(
        "--payload", required=True, help="Contenido del intercambio"
    )
    fed_receive.add_argument("--risk", default="low", help="low|medium|high|critical")
    fed_receive.add_argument(
        "--domain", default="federated", help="Dominio del candidato"
    )
    ...
    fed_exchanges = fed_subparsers.add_parser(
        "exchanges", help="Lista el log de intercambios"
    )
    fed_exchanges.add_argument(
        "--node-id", dest="node_id", default=None, help="Filtrar por nodo"
    )
    ...
    qualia_parser = subparsers.add_parser(
        "qualia", help="QualiaBus: experiencias neuronales y estado circulatorio"
    )
    qualia_parser.add_argument(
        "--db", default="triade/memory/triade.db", help="Ruta de base SQLite"
    )
    qualia_sub = qualia_parser.add_subparsers(dest="qualia_command")

    qualia_state = qualia_sub.add_parser(
        "state", help="Muestra estado QualiaBus reciente"
    )
    ...
    qualia_exp = qualia_sub.add_parser(
        "experiences", help="Lista experiencias neuronales Qualia"
    )
    ...
    qualia_pub = qualia_sub.add_parser(
        "publish-test", help="Publica una experiencia Qualia de prueba"
    )
    qualia_pub.add_argument(
        "--run-id", default="qualia-cli-test", help="run_id de prueba"
    )
    qualia_pub.add_argument(
        "--observation",
        default="Experiencia de prueba QualiaBus desde CLI.",
        help="Observación",
    )
    qualia_pub.add_argument(
        "--proposed-learning",
        default="",
        help="Contenido opcional para candidato de aprendizaje",
    )
    ...
    neuron_common.add_argument(
        "--db", default="triade/memory/triade.db", help="Ruta de base SQLite"
    )
    neuron_common.add_argument(
        "--runs-dir", default="runs", help="Directorio de runs auditables"
    )
    neuron_parser = subparsers.add_parser(
        "neuron",
        help="Neuronas internas con identidad y evidencia",
        parents=[neuron_common],
    )
    neuron_sub = neuron_parser.add_subparsers(dest="neuron_command")
    neuron_list = neuron_sub.add_parser(
        "list",
        help="Lista neuronas con identidad diferenciada",
        parents=[neuron_common],
    )
    neuron_list.add_argument("--limit", type=int, default=50, help="Cantidad máxima")
    neuron_show = neuron_sub.add_parser(
        "show", help="Muestra detalle de una neurona", parents=[neuron_common]
    )
    neuron_show.add_argument("name", help="Nombre de la neurona")
    neuron_show.add_argument(
        "--limit", type=int, default=20, help="Cantidad máxima de evidencias"
    )
    neuron_audit = neuron_sub.add_parser(
        "audit-stable", help="Audita neuronas stable débiles", parents=[neuron_common]
    )
    neuron_audit.add_argument(
        "--limit", type=int, default=300, help="Cantidad máxima a inspeccionar"
    )
    neuron_audit.add_argument(
        "--apply", action="store_true", help="Aplica la auditoría de forma explícita"
    )

    neuron_missions_common = argparse.ArgumentParser(add_help=False)
    neuron_missions_common.add_argument(
        "--db", default="triade/memory/triade.db", help="Ruta de base SQLite"
    )
    neuron_missions_common.add_argument(
        "--runs-dir", default="runs", help="Directorio de runs auditables"
    ...
    neuron_missions_common.add_argument(
        "--limit", type=int, default=500, help="Cantidad máxima a inspeccionar"
    )
    neuron_missions_parser = subparsers.add_parser(
        "neuron-missions",
        help="Backfill y doctor de misiones neuronales",
        parents=[neuron_missions_common],
    )
    neuron_missions_sub = neuron_missions_parser.add_subparsers(
        dest="neuron_missions_command"
    )
    neuron_missions_sub.add_parser(
        "backfill",
        help="Crea misiones faltantes desde neuronas activas",
        parents=[neuron_missions_common],
    )
    neuron_missions_sub.add_parser(
        "list", help="Lista misiones neuronales", parents=[neuron_missions_common]
    )
    neuron_missions_sub.add_parser(
        "doctor",
        help="Diagnóstico operativo de misiones neuronales",
        parents=[neuron_missions_common],
    )

    workers_parser = subparsers.add_parser(
        "workers", help="Triade Living Workers en segundo plano controlado"
    )
    ...
    workers_parser.add_argument(
        "--runs-dir",
        default="runs/background",
        help="Directorio de artefactos background",
    )
    ...
    workers_once.add_argument(
        "--dry-run", action="store_true", help="Planifica sin ejecutar mutaciones"
    )
    workers_once.add_argument(
        "--task-timeout", type=float, default=30.0, help="Timeout por tarea"
    )

    workers_start = workers_sub.add_parser(
        "start", help="Ejecuta worker daemon acotado"
    )
    workers_start.add_argument(
        "--max-iterations", type=int, default=5, help="Máximo de iteraciones"
    )
    workers_start.add_argument(
        "--sleep", type=float, default=2.0, help="Segundos entre iteraciones"
    )
    workers_start.add_argument(
        "--dry-run", action="store_true", help="Planifica sin ejecutar mutaciones"
    )
    ...
    workers_daemon = workers_sub.add_parser(
        "daemon",
        help="Ejecuta workers en modo daemon controlado (sin shell, sin red externa)",
    )
    workers_daemon.add_argument(
        "--max-iterations",
        type=int,
        default=10,
    ...
    workers_daemon.add_argument(
        ...
        type=float,
        default=5.0,
        help="Segundos entre iteraciones (default: 5.0)",
    )
    workers_daemon.add_argument(
        "--task-timeout", type=float, default=30.0, help="Timeout por tarea"
    )
    ...
    runtime_common.add_argument(
        "--db", default="triade/memory/triade.db", help="Ruta de base SQLite"
    )
    runtime_common.add_argument(
        "--runs-dir", default="runs/background", help="Directorio de artefactos runtime"
    )
    runtime_common.add_argument(
        "--mode",
        default=None,
        help="observe_only|learn_candidates|execute_missions|full_local",
    )
    runtime_common.add_argument(
        "--interval-seconds",
        type=int,
        default=30,
        help="Intervalo entre ciclos en modo start",
    )
    runtime_common.add_argument(
        "--max-cycles", type=int, default=0, help="Máximo de ciclos en modo start"
    )
    runtime_common.add_argument(
        "--limit",
        type=int,
        default=20,
        help="Cantidad máxima para events/context/report",
    )
    runtime_common.add_argument(
        "--user-input", default="", help="Entrada para construir contexto vivo"
    )
    runtime_parser = subparsers.add_parser(
        "runtime", help="Runtime interno 24/7 y estado vivo", parents=[runtime_common]
    )
    runtime_sub = runtime_parser.add_subparsers(dest="runtime_command")
    runtime_sub.add_parser(
        "status", help="Estado actual del runtime", parents=[runtime_common]
    )
    runtime_sub.add_parser(
        "once", help="Ejecuta un ciclo interno único", parents=[runtime_common]
    )
    runtime_sub.add_parser(
        "start", help="Inicia runtime continuo controlado", parents=[runtime_common]
    )
    runtime_sub.add_parser(
        "stop", help="Detiene runtime continuo", parents=[runtime_common]
    )
    runtime_sub.add_parser(
        "events", help="Muestra eventos internos recientes", parents=[runtime_common]
    )
    runtime_sub.add_parser(
        "heartbeat",
        help="Muestra heartbeat cognitivo del runtime",
        parents=[runtime_common],
    )
    runtime_sub.add_parser(
        "blood",
        help="Muestra estado de sangre cognitiva Ollama",
        parents=[runtime_common],
    )
    runtime_sub.add_parser(
        "context", help="Muestra contexto vivo resumido", parents=[runtime_common]
    )
    runtime_sub.add_parser(
        "report", help="Muestra reporte vivo resumido", parents=[runtime_common]
    )

    # ── Always-On ────────────────────────────────────────────────────────
    ao_parser = subparsers.add_parser(
        "always-on", help="Controla ALWAYS-ON runtime persistente"
    )
    ...
    ao_enable.add_argument(
        "--mode",
        default=None,
        help="observe_only|execute_missions|balanced_background|full_local_guarded",
    )
    ao_enable.add_argument(
        "--interval", type=int, default=None, help="Intervalo en segundos"
    )
    ao_enable.add_argument(
        "--safe-only",
        dest="safe_only",
    ...
    ao_enable.add_argument(
        ...
        type=bool,
        default=None,
        help="Requerir Ollama",
    )
    ao_enable.add_argument(
        "--self-test-every",
        dest="self_test_every",
        type=int,
        default=None,
        help="Self-test cada N ciclos",
    )
    ao_enable.add_argument(
        "--config", default="triade.yml", help="Ruta de configuración"
    )
    ao_sub.add_parser(
        "disable", help="Deshabilita ALWAYS-ON en triade.yml"
    ).add_argument("--config", default="triade.yml")
    ...
    relay_job = relay_subparsers.add_parser(
        "create-job", help="Crea job para un nodo web"
    )
    relay_job.add_argument("node_id", help="Nodo destino")
    relay_job.add_argument(
        "--task", default="browser_benchmark", help="echo|sha256|browser_benchmark"
    )
    relay_job.add_argument("--payload", default="{}", help="JSON de entrada")
    relay_job.add_argument(
        "--seconds", type=float, default=2.0, help="Duracion para benchmark"
    )

    relay_subparsers.add_parser(
        "sync-nodes", help="Sincroniza nodos online del relay a la federacion local"
    )

    relay_benchmark = relay_subparsers.add_parser(
        "benchmark", help="Mide nodos browser y guarda su score en la federacion local"
    )
    relay_benchmark.add_argument(
        "--seconds", type=float, default=2.0, help="Duracion de cada benchmark"
    )
    relay_benchmark.add_argument(
        "--wait-timeout",
        type=float,
        default=45.0,
        help="Tiempo maximo de espera por job",
    )

    relay_feed = relay_subparsers.add_parser(
        "model-feed",
        help="Resume nodos que alimentan la planificacion local de modelos",
    )
    relay_feed.add_argument("--min-tier", default="low", help="low|medium|high")

    relay_preprocess = relay_subparsers.add_parser(
        "preprocess-text",
        help="Preprocesa contexto en nodos browser para alimentar modelos locales",
    )
    relay_preprocess.add_argument("text", help="Texto/contexto a preparar")
    relay_preprocess.add_argument(
        "--max-chunk-chars", type=int, default=1200, help="Tamano maximo de chunk"
    )
    relay_preprocess.add_argument(
        "--wait-timeout",
        type=float,
    ...
    if args.command == "chat":
        ...
        while True:
            ...
            print(
                f"hipotálamo: {hyp['provider']}:{hyp['name']} ok={hyp['ok']} quality={hyp.get('quality_score')}"
            )
            print(
                f"central: {cen['provider']}:{cen['name']} ok={cen['ok']} quality={cen.get('quality_score')}"
            )
        return
