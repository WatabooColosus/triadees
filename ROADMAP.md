...
- **Fase OBS · ✅ COMPLETA** — Observabilidad y Trazabilidad: `error_bus.py` registra errores internos en `worker_events` reemplazando silencios internos, `life_pulse` registra fallos del continuous runner sin matar el hilo en silencio, `record_learning_usage_from_output` usa matching explícito (candidate_ids, semantic_doc_ids, evidence_refs) y heurístico marcado como fallback, trazabilidad explícita en `output.memory_diff.traceability` (candidatos, documentos semánticos, misiones, evidencia, match_sources, heuristic_matches), MissionPlanner emite `reason/source/planner_score` y usa baseline condicional (pulse_check siempre, otros solo si hay work), endpoints OBS (`/api/internal/errors`, `/api/workers/events`, `/api/neurons/missions/{id}/cycles|evidence|scores`), panel UI de observabilidad, tests de error_bus/continuous runner/planner/E2E.

**Deuda pendiente tras OBS:** consolidar las superficies FastAPI duplicadas bajo `single_port_app`, ampliar tests de UI con navegador, definir retención/rotación de `worker_events`, y documentar una política de severidad para distinguir errores críticos de degradaciones recuperables.
