...
- Runtime interno 24/7: existe `InternalRuntimeSupervisor` con Always-On activo en esta instalación local. El modo configurado por defecto es `full_local_guarded`; Resource Governor puede degradar el modo efectivo a `balanced_background`, `light_background` u `observe_only` si los recursos no alcanzan. El chat consulta el estado vivo mediante `build_living_context_for_chat()` y `build_living_report()`.
