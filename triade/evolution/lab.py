...
class EvolutionPolicy:
    required_domains: tuple[str, ...] = (
        "reasoning", "code", "mathematics", "science", "planning",
        "longitudinal_memory", "social_understanding", "causality",
        "safety", "tool_use", "rule_adaptation", "long_horizon",
    )
...
class EvolutionLab:
    ...
    def __init__(self, db_path: str | Path = "triade/memory/triade.db", *, signing_key: bytes | None = None) -> None:
    ...
    def create_campaign(
        self, title: str, hypothesis: str, baseline_version: str,
        candidate_version: str, policy: EvolutionPolicy | None = None,
    ) -> dict[str, Any]:
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                (campaign_id, title, hypothesis, baseline_version, candidate_version,
                 _canonical(asdict(p)), now, now),
            )
        return self.campaign(campaign_id)

    def campaign(self, campaign_id: str) -> dict[str, Any]:
        with self._connect() as conn:
            row = conn.execute("SELECT * FROM evolution_campaigns WHERE campaign_id=?", (campaign_id,)).fetchone()
        if row is None:
    ...
    def freeze_battery(self, campaign_id: str, cases: Iterable[dict[str, Any]]) -> dict[str, Any]:
        ...
        domains = sorted({str(case.get("domain", "")) for case in material if case.get("domain")})
        ...
        manifest = {"domains": domains, "case_hashes": case_hashes, "count": len(material)}
        battery_id = f"battery-{uuid.uuid4().hex[:12]}"
        with self._connect() as conn:
            conn.execute(
                "INSERT INTO frozen_batteries VALUES (?,?,?,?,?,1,?)",
                (battery_id, campaign_id, _sha(manifest), _canonical(domains), _canonical(case_hashes), _now()),
            )
        return {"battery_id": battery_id, "manifest_hash": _sha(manifest), "domains": domains,
                "case_count": len(case_hashes), "sealed": True}

    def record_evidence(
        self, campaign_id: str, stage: Stage, kind: str, payload: dict[str, Any],
        *, source: str, independent: bool = False,
    ) -> dict[str, Any]:
        campaign = self.campaign(campaign_id)
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                (evidence_id, campaign_id, int(stage), kind, _canonical(payload), digest,
                 source, int(independent), _now()),
            )
        return {"evidence_id": evidence_id, "payload_hash": digest, "independent": independent}

    def register_artifact(
        self, campaign_id: str, kind: str, path: str | Path, *,
        parent_sha256: str | None = None, metadata: dict[str, Any] | None = None,
    ) -> dict[str, Any]:
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                (artifact_id, campaign_id, kind, str(artifact_path), digest, parent_sha256,
                 _canonical(metadata or {}), _now()),
            )
        return {"artifact_id": artifact_id, "sha256": digest, "status": "candidate"}

    def charge_resources(
        self, campaign_id: str, *, gpu_minutes: float = 0, experiments: int = 0,
        storage_mb: float = 0,
    ) -> dict[str, Any]:
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                (f"usage-{uuid.uuid4().hex[:12]}", campaign_id, gpu_minutes, experiments, storage_mb, _now()),
            )
            row = conn.execute(
        ...
        usage = {"gpu_minutes": row["gpu"], "experiments": row["exp"], "storage_mb": row["storage"]}
    ...
    def evaluate_stage(self, campaign_id: str) -> StageDecision:
        ...
        if stage == Stage.MEASUREMENT:
            with self._connect() as conn:
                battery = conn.execute("SELECT * FROM frozen_batteries WHERE campaign_id=?", (campaign_id,)).fetchone()
            if battery is None:
            ...
            if not comparison:
                ...
                low = {k: v for k, v in scores.items() if float(v) < policy["minimum_domain_score"]}
                overall = float(comparison.get("candidate_overall", 0))
                improvement = overall - float(comparison.get("baseline_overall", 0))
                regressions = comparison.get("regressions", {})
                metrics.update({"overall": overall, "improvement": improvement, "low_domains": low,
                                "missing_domains": sorted(missing), "regressions": regressions})
                if missing or low: reasons.append("cobertura o score de dominios insuficiente")
                if overall < policy["minimum_overall_score"]: reasons.append("score global insuficiente")
                if improvement < policy["minimum_improvement"]: reasons.append("mejora contra baseline insuficiente")
                if any(abs(float(v)) > policy["maximum_regression"] for v in regressions.values()):
            ...
            contexts = {p for e in evidence for p in json.loads(e["payload_json"]).get("transfer_contexts", [])}
            metrics.update({"lessons": len(lessons), "independent_evidence": independent,
                            "transfer_contexts": len(contexts)})
            if not lessons: reasons.append("falta lección reproducible")
            if independent < policy["minimum_independent_evidence"]: reasons.append("evidencia independiente insuficiente")
            if len(contexts) < policy["minimum_transfer_contexts"]: reasons.append("transferencia insuficiente")
        elif stage == Stage.ADAPTER:
            required = {"dataset_split", "adapter_training", "ood_evaluation", "forgetting_evaluation", "canary"}
            present = {e["kind"] for e in evidence}
            metrics["present"] = sorted(present)
            if required - present: reasons.append(f"faltan controles de adaptador: {sorted(required - present)}")
            canary = self._latest_payload(evidence, "canary") or {}
            if int(canary.get("observations", 0)) < policy["minimum_canary_observations"]:
                reasons.append("canary insuficiente")
            if not canary.get("rollback_ready", False): reasons.append("rollback no demostrado")
            if not self._has_artifact(campaign_id, "adapter"): reasons.append("falta artefacto de adaptador firmado")
        elif stage == Stage.RESEARCH:
            cycle = self._latest_payload(evidence, "scientific_cycle") or {}
            required = {"question", "sources", "hypothesis", "prediction", "experiment", "result", "refutation", "update"}
            missing = sorted(k for k in required if not cycle.get(k))
            metrics["missing_fields"] = missing
            if missing: reasons.append("ciclo científico incompleto")
            if any(not s.get("url") or not s.get("retrieved_at") for s in cycle.get("sources", [])):
                reasons.append("fuentes sin procedencia o fecha")
            if cycle.get("memory_status") not in {"candidate", "rejected"}:
                reasons.append("investigación no puede entrar directamente como memoria estable")
        elif stage == Stage.LONG_HORIZON:
            run = self._latest_payload(evidence, "long_horizon_run") or {}
            checkpoints = run.get("checkpoints", [])
            metrics.update({"checkpoints": len(checkpoints), "replans": run.get("replans", 0),
                            "recovered_after_restart": run.get("recovered_after_restart", False)})
            if len(checkpoints) < policy["minimum_long_horizon_checkpoints"]: reasons.append("checkpoints insuficientes")
            if not run.get("stagnation_detection", False): reasons.append("falta detección de estancamiento")
            if not run.get("uncertainty_estimation", False): reasons.append("falta estimación de incertidumbre")
            if not run.get("recovered_after_restart", False): reasons.append("recuperación tras reinicio no demostrada")
            if not self.charge_resources(campaign_id)["within_budget"]: reasons.append("presupuesto excedido")
        else:
            reports = [e for e in evidence if e["kind"] == "external_report" and e["independent"]]
            metrics["external_reports"] = len(reports)
            valid = [e for e in reports if self._valid_external_report(json.loads(e["payload_json"]))]
        ...
        decision = StageDecision(stage, not reasons, tuple(reasons), metrics, _sha([e["payload_hash"] for e in evidence]))
    ...
    def advance(self, campaign_id: str) -> dict[str, Any]:
        ...
        with self._connect() as conn:
            conn.execute("UPDATE evolution_campaigns SET stage=?, status=?, updated_at=? WHERE campaign_id=?",
                         (next_stage, status, _now(), campaign_id))
        return {"advanced": True, "campaign": self.campaign(campaign_id),
                "decision": self._decision_dict(decision)}

    def reject(self, campaign_id: str, reason: str) -> dict[str, Any]:
        if not reason.strip(): raise ValueError("se requiere una razón")
        self.record_evidence(campaign_id, Stage(self.campaign(campaign_id)["stage"]), "rejection",
                             {"reason": reason}, source="governance")
        with self._connect() as conn:
            conn.execute("UPDATE evolution_campaigns SET status='rejected', updated_at=? WHERE campaign_id=?",
                         (_now(), campaign_id))
            conn.execute("UPDATE evolution_artifacts SET status='rejected' WHERE campaign_id=?", (campaign_id,))
        return self.campaign(campaign_id)
    ...
    def report(self, campaign_id: str) -> dict[str, Any]:
        ...
        with self._connect() as conn:
            decisions = [dict(r) for r in conn.execute(
                "SELECT * FROM evolution_stage_decisions WHERE campaign_id=? ORDER BY created_at", (campaign_id,))]
            artifacts = [dict(r) for r in conn.execute(
                "SELECT * FROM evolution_artifacts WHERE campaign_id=? ORDER BY created_at", (campaign_id,))]
        body = {"campaign": campaign, "decisions": decisions, "artifacts": artifacts,
                "resource_usage": self.charge_resources(campaign_id)}
        body["sha256"] = _sha(body)
        if self.signing_key:
            body["signature"] = hmac.new(self.signing_key, body["sha256"].encode(), hashlib.sha256).hexdigest()
        return body
    ...
    @staticmethod
    def _latest_payload(evidence: list[dict[str, Any]], kind: str) -> dict[str, Any] | None:
    ...
    def _has_artifact(self, campaign_id: str, kind: str) -> bool:
        with self._connect() as conn:
            return conn.execute("SELECT 1 FROM evolution_artifacts WHERE campaign_id=? AND kind=?", (campaign_id, kind)).fetchone() is not None

    def _valid_external_report(self, payload: dict[str, Any]) -> bool:
        required = {"evaluator", "suite", "score", "report_hash", "signature"}
        return required <= payload.keys() and bool(payload.get("evaluator")) and 0 <= float(payload.get("score", -1)) <= 1

    def _persist_decision(self, campaign_id: str, decision: StageDecision) -> None:
        with self._connect() as conn:
            conn.execute("INSERT INTO evolution_stage_decisions VALUES (?,?,?,?,?,?,?,?)",
                         (f"decision-{uuid.uuid4().hex[:12]}", campaign_id, int(decision.stage), int(decision.passed),
                          _canonical(decision.reasons), _canonical(decision.metrics), decision.evidence_hash, _now()))

    @staticmethod
    def _decision_dict(decision: StageDecision) -> dict[str, Any]:
        return {"stage": int(decision.stage), "stage_name": decision.stage.name.lower(), "passed": decision.passed,
                "reasons": list(decision.reasons), "metrics": decision.metrics,
