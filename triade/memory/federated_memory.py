...
class FederatedMemory:
    ...
    def search(
        self, domain: str = "", source_node: str = "", limit: int = 50
    ) -> list[dict]:
    ...
    def doctor(self) -> dict:
        total = self._conn.execute(
            "SELECT COUNT(*) as c FROM federated_memory_items WHERE status='active'"
        ).fetchone()["c"]
        nodes = self._conn.execute(
            "SELECT COUNT(DISTINCT source_node) as c FROM federated_memory_items"
        ).fetchone()["c"]
        merges = self._conn.execute(
            "SELECT COUNT(*) as c FROM federated_merge_log"
        ).fetchone()["c"]
        return {
            "active_items": total,
            "source_nodes": nodes,
