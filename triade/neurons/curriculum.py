...
def terms(text: str) -> set[str]:
    stop = {"para", "como", "que", "una", "con", "por", "del", "the", "and", "mission", "neurona"}
    return {word for word in re.findall(r"[a-záéíóúñ0-9]{3,}", text.lower().replace("_", " ")) if word not in stop}


def source_domain(source_ref: str) -> str:
    return (urllib.parse.urlparse(source_ref).hostname or source_ref.split(":", 1)[0]).lower()


def relevant_material(rows: list[dict[str, Any]], objective: str, domain: str, *, limit: int = 5) -> list[dict[str, Any]]:
