...
SENSITIVE = re.compile(r"\b(password|contrase(?:ña|na)|token|credencial|malware|exploit|arma|autolesi[oó]n|datos privados)\b", re.I)
FACTUAL = re.compile(r"\b(qué|que|quién|quien|cuál|cual|cómo|como|cuándo|cuando|dónde|donde|por qué|porque|explica|investiga)\b", re.I)
...
class AutonomousResearchEngine:
    ...
    def ingest_result(self, query: str, result: dict[str, Any], *, trigger: str = "explicit_request") -> dict[str, Any]:
        ...
        with self._connect() as conn:
            conn.execute("INSERT INTO autonomous_research_runs VALUES (?,?,?,?,?,?,?,?,?)", (
                rid, query, trigger, status, canonical, digest, json.dumps(candidates), json.dumps(contradictions),
                datetime.now(timezone.utc).isoformat(),
            ))
        return {"research_id": rid, "status": status, "query": query, "trigger": trigger, "sources": sources,
                "source_hash": digest, "candidate_ids": candidates, "contradictions": contradictions,
                "stable_memory_written": False}

    def _queries_today(self) -> int:
        today = datetime.now(timezone.utc).date().isoformat()
        with self._connect() as conn:
            return int(conn.execute("SELECT COUNT(*) FROM autonomous_research_runs WHERE created_at LIKE ?", (today + "%",)).fetchone()[0])
    ...
    def _contradictions(sources: list[dict[str, Any]]) -> list[str]:
        ...
        for i, left in enumerate(excerpts):
            for right in excerpts[i + 1:]:
                if (" no " in left and " sí " in right) or (" sí " in left and " no " in right):
                    flags.append("Fuentes contienen afirmaciones potencialmente opuestas; requiere verificación independiente.")
                    return flags
        return flags

    @staticmethod
    def _terms(text: str) -> set[str]:
        stop={"para","como","qué","que","una","unos","las","los","del","con","por","internet","web"}
        return {w for w in re.findall(r"[a-záéíóúñ0-9]{3,}",text.lower()) if w not in stop}
