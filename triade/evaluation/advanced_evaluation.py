...
class MutationTester:
    ...
    def doctor(self) -> dict:
        total = self._conn.execute(
            "SELECT COUNT(*) as c FROM mutation_tests"
        ).fetchone()["c"]
        killed = self._conn.execute(
            "SELECT COUNT(*) as c FROM mutation_tests WHERE killed=1"
        ).fetchone()["c"]
        return {
            "total_mutations": total,
            "killed": killed,
...
class RegressionDetector:
    ...
    def doctor(self) -> dict:
        total = self._conn.execute(
            "SELECT COUNT(*) as c FROM regression_events"
        ).fetchone()["c"]
        open_events = self._conn.execute(
            "SELECT COUNT(*) as c FROM regression_events WHERE status='open'"
        ).fetchone()["c"]
...
class QualityCompositor:
    ...
    def evaluate(self, target_name: str, dimensions: dict[str, float]) -> dict:
        ...
        overall = sum(
            _clamp(dimensions.get(k, 0.0)) * w for k, w in self.WEIGHTS.items()
        )
    ...
    def doctor(self) -> dict:
        total = self._conn.execute(
            "SELECT COUNT(*) as c FROM quality_composites"
        ).fetchone()["c"]
        avg = (
            self._conn.execute(
                "SELECT AVG(overall_score) as a FROM quality_composites"
            ).fetchone()["a"]
            or 0
        )
