...
class QualiaRouter:
    ...
    def route(self, experience: NeuronExperience) -> QualiaBundle:
        ...
        return QualiaBundle(
            ...
            signal=signal,
            central_packet=central,
            storage_packet=storage,
    ...
    def to_signal(self, experience: NeuronExperience) -> QualiaSignal:
        ...
        emotional = (
            experience.emotional_signal
            if isinstance(experience.emotional_signal, dict)
            else {}
        )
        ...
        curiosity = _clamp(
            0.25
            + (0.35 if experience.extracted_pattern else 0.0)
            + (0.25 if experience.proposed_learning else 0.0)
        )
        intensity = _clamp(
            max(experience.confidence, experience.usefulness, risk_value)
        )
        signal_type = (
            "learning_candidate"
            if experience.proposed_learning
            else "neuron_observation"
        )
    ...
    def to_central_packet(self, experience: NeuronExperience) -> CentralKnowledgePacket:
        claim = (
            experience.extracted_pattern or experience.observation or experience.mission
        )
        hypothesis = (
            experience.proposed_learning
            or experience.extracted_pattern
            or "Revisar experiencia neuronal antes de usarla como contexto."
        )
        validation = (
            "verified_context_allowed"
            if experience.confidence >= 0.85 and str(experience.risk) == "low"
        ...
        return CentralKnowledgePacket(
            ...
            if validation == "verified_context_allowed"
            else "hypothesis",
        )
    ...
    def to_storage_packet(self, experience: NeuronExperience) -> StorageMemoryPacket:
        content = "\n".join(
            ...
            for part in [
                ...
                f"observation: {experience.observation}"
                if experience.observation
                else "",
                f"pattern: {experience.extracted_pattern}"
                if experience.extracted_pattern
                else "",
                f"proposed_learning: {experience.proposed_learning}"
                if experience.proposed_learning
                else "",
            ]
            if part
    ...
    def to_learning_candidate(
        self, experience: NeuronExperience
    ) -> dict[str, Any] | None:
