        ...
        experiences.append(
            NeuronExperience(
                ...
                extracted_pattern=str(
                    post.get("title")
                    or post.get("candidate_id")
                    or "candidato post-run"
                ),
                proposed_learning=str(
                    post.get("normalized_summary") or post.get("content") or ""
                ),
        ...
        experiences.append(
            NeuronExperience(
                ...
                mission=str(
                    ...
                    or activation.get("name")
                    or "observación experimental"
                ),
                source=str(activation.get("name") or "experimental_neuron_activity"),
                source_type="experimental_neuron_activity",
                observation="; ".join(str(item) for item in diagnosis)
                or "Neurona experimental activada en modo diagnóstico.",
                ...
                evidence_refs=[
                    f"run:{run_id}",
                    f"neuron_activity:{activation.get('neuron_id')}",
                ]
                if activation.get("neuron_id")
                else [f"run:{run_id}"],
            )
        )
    ...
    for candidate in background_neuron_candidates or []:
        ...
        experiences.append(
            NeuronExperience(
                ...
                extracted_pattern=str(
                    candidate.get("policy") or "candidate_requires_review"
                ),
    ...
    if gate.get("modified"):
        experiences.append(
            NeuronExperience(
                ...
                observation=str(
                    gate.get("reason") or "OutputGate modificó la respuesta."
                ),
    ...
    for event in (neuron_orchestration or {}).get("system_events") or []:
        ...
        experiences.append(
            NeuronExperience(
                ...
                observation=str(
                    event.get("message") or event.get("type") or "evento del sistema"
                ),
                ...
                evidence_refs=[
                    f"run:{run_id}",
                    f"system_event:{event.get('type') or 'unknown'}",
                ],
            )
        )
    return experiences
...
def qualia_context_for_memory(
    db_path: str | Path, run_id: str | None = None, limit: int = 5
) -> dict[str, Any]:
    try:
        store = QualiaStore(db_path=db_path)
        latest_state = store.latest_state(run_id=run_id)
        packets = store.list_central_packets(
            run_id=run_id, limit=limit, statuses={"hypothesis", "verified_context"}
        )
        ...
        return {
            "status": "unavailable",
            "error": str(exc),
            "policy": "QualiaBus degradó sin romper recall.",
        }
