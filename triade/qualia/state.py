...
def compute_qualia_state(
    run_id: str, signals: list[dict[str, Any]], experiences: list[dict[str, Any]]
) -> QualiaState:
    count = len(signals)
    dominant = "none"
    if signals:
        dominant = Counter(
            str(row.get("signal_type") or "observation") for row in signals
        ).most_common(1)[0][0]
    ...
    usefulness = (
        round(mean([float(row.get("usefulness") or 0.0) for row in experiences]), 3)
        if experiences
        else 0.0
    )
    ...
    coherence = (
        round(max(0.0, min(1.0, (confidence + usefulness + (1.0 - risk)) / 3.0)), 3)
        if count
        else 0.0
    )
    if risk >= 0.75:
