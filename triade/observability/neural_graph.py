...
def build_neural_graph(db_path: Path, *, limit: int = 5000) -> tuple[list[GraphNode], list[GraphEdge]]:
    ...
    try:
        ...
        for table, (kind, identity) in RUNTIME_TABLES.items():
            ...
            if identity not in columns:
                continue
            for row in connection.execute(f"SELECT * FROM {table} ORDER BY rowid DESC LIMIT ?", (limit,)):
                value = row[identity]
                if value is None:
                    continue
                node_id = f"{kind}:{value}"
                nodes.setdefault(node_id, GraphNode(node_id, kind, str(value), _state_from_row(row), _safe_metadata(row)))
                edges.append(GraphEdge(table_id, node_id, "contains_runtime_record", f"sqlite:{table}"))
...
def _safe_metadata(row: sqlite3.Row) -> dict[str, object]:
    allowed = {"status", "state", "outcome", "created_at", "updated_at", "started_at", "finished_at", "task_type"}
    return {key: row[key] for key in row.keys() if key in allowed and row[key] is not None}


def _append_relations(table: str, row: sqlite3.Row, source: str, nodes: dict[str, GraphNode], edges: list[GraphEdge]) -> None:
    for field, kind, relation in (("run_id", "run", "participates_in"), ("neuron_id", "neuron", "uses_neuron"), ("task_id", "task", "references_task")):
