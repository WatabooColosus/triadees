...
HealthState = Literal["healthy", "degraded", "stalled", "recovering", "critical", "stopped"]
...
class ServiceHealth:
    def __init__(self, db_path: str | Path = "triade/memory/triade.db", *, heartbeat_stale_seconds: int = 180,
                 cycle_stale_seconds: int = 900, disk_critical_gb: float = 2, ram_critical_gb: float = 1) -> None:
    ...
    def inspect(self, *, process_running: bool | None = None, ollama_probe: dict[str, Any] | None = None) -> RuntimeHealth:
        ...
        try:
            with sqlite3.connect(self.db_path, timeout=2) as conn:
                ...
                if "worker_tasks" in tables:
                    counts = {str(row[0]): int(row[1]) for row in conn.execute("SELECT status,COUNT(*) FROM worker_tasks GROUP BY status")}
                    metrics["queue"] = counts
                    oldest = conn.execute("SELECT created_at FROM worker_tasks WHERE status IN ('pending','running','claimed') ORDER BY created_at LIMIT 1").fetchone()
                    age = self._age_seconds(oldest[0], now) if oldest else None
                    metrics["oldest_active_task_seconds"] = age
                    if age is not None and age > self.cycle_stale_seconds and counts.get("completed", 0) == 0:
                        reasons.append("queue_not_progressing")
                        stalled = True
                if "worker_runs" in tables:
                    run = conn.execute("SELECT status,started_at,finished_at FROM worker_runs ORDER BY id DESC LIMIT 1").fetchone()
                    metrics["last_worker_run"] = dict(run) if run else None
                    if run and run["status"] == "running" and self._age_seconds(run["started_at"], now) > self.cycle_stale_seconds:
                        reasons.append("worker_cycle_stalled")
                        stalled = True
                if "worker_state" in tables:
                    heartbeat = conn.execute("SELECT updated_at,value_json FROM worker_state WHERE key IN ('workers','scheduler_heartbeat') ORDER BY updated_at DESC LIMIT 1").fetchone()
                    age = self._age_seconds(heartbeat["updated_at"], now) if heartbeat else None
                    metrics["heartbeat_age_seconds"] = age
                    if age is None or age > self.heartbeat_stale_seconds:
                        reasons.append("heartbeat_stale")
                        stalled = True
                if "worker_events" in tables:
                    repeated = conn.execute("SELECT COUNT(*) FROM worker_events WHERE status IN ('error','failed') AND created_at>=datetime('now','-15 minutes')").fetchone()[0]
        ...
        if float(resources.get("disk", {}).get("free_gb") or 0) < self.disk_critical_gb:
            ...
            db_critical = True
        if float(resources.get("memory", {}).get("available_gb") or 0) < self.ram_critical_gb:
        ...
        if process_running is False:
            ...
            state = "healthy"
        return RuntimeHealth(state, tuple(dict.fromkeys(reasons)), metrics, now.isoformat())
    ...
    def _ollama_probe() -> dict[str, Any]:
        try:
            ...
            return dict(result) if isinstance(result, dict) else {"ok": False, "error_type": "invalid_probe_result"}
