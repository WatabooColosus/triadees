...
class NeuronRollback:
    ...
    def snapshot(
        self, target_type: str, target_id: str, state: dict, action: str = "pre_change"
    ) -> dict:
    ...
    def doctor(self) -> dict:
        total = self._conn.execute(
            "SELECT COUNT(*) as c FROM neuron_rollback_log"
        ).fetchone()["c"]
        pending = self._conn.execute(
            "SELECT COUNT(*) as c FROM neuron_rollback_log WHERE status='snapshotted'"
        ).fetchone()["c"]
        rolled = self._conn.execute(
            "SELECT COUNT(*) as c FROM neuron_rollback_log WHERE status='rolled_back'"
        ).fetchone()["c"]
