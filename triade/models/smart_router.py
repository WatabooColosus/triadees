...
class SmartModelRouter:
    ...
    def select(
        ...
        if not scored:
            ...
            return {
                "model": fallback,
                "role": role,
                "reason": "fallback_no_hardware_match",
                "score": 0.0,
    ...
    def record_result(
        ...
        if row:
            ...
            avg_lat = (row["avg_latency_ms"] * row["total_calls"] + latency_ms) / max(
                total, 1
            )
            avg_tok = (row["avg_tokens"] * row["total_calls"] + tokens_used) / max(
                total, 1
            )
            self._conn.execute(
    ...
    def doctor(self) -> dict:
        models = self._conn.execute(
            "SELECT COUNT(DISTINCT model_name) as c FROM model_performance"
        ).fetchone()["c"]
        logs = self._conn.execute(
            "SELECT COUNT(*) as c FROM model_route_log"
        ).fetchone()["c"]
