...
class IsolatedInstaller:
    def __init__(
        ...
        root: str | Path = "artifacts/environments",
    ) -> None:
    ...
    def install(
        self, package: str, *, goal_id: str, approved: bool, approved_by: str
    ) -> dict[str, Any]:
        ...
        try:
            ...
            (env / "triade-install-manifest.json").write_text(
                json.dumps({**manifest, "sha256": digest}, indent=2), encoding="utf-8"
            )
            with sqlite3.connect(self.db_path) as conn:
                conn.execute(
                    "UPDATE installer_attempts SET status='installed_isolated',finished_at=CURRENT_TIMESTAMP,manifest_sha256=? WHERE id=?",
                    (digest, attempt),
                )
            return {
                "status": "completed",
