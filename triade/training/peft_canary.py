...
class PeftCanaryServer:
    ...
    def __init__(self, db_path: str | Path = "triade/memory/triade.db",
                 adapters_root: str | Path = "artifacts/adapters") -> None:
    ...
    def _resolve(self, adapter_path: str) -> Path:
        ...
        if self.adapters_root not in path.parents or not (path / "adapter_model.safetensors").is_file():
        ...
        actual = hashlib.sha256((path / "adapter_model.safetensors").read_bytes()).hexdigest()
    ...
    def _load(self, adapter_path: str) -> tuple[Any, Any, Path]:
        ...
        with self._lock:
            ...
            manifest = json.loads((path / "triade_adapter_manifest.json").read_text(encoding="utf-8"))
            ...
            self._model, self._tokenizer, self._loaded_adapter = model, tokenizer, str(path)
            return model, tokenizer, path

    def generate(self, adapter_path: str, prompt: str, *, max_new_tokens: int = 64) -> dict[str, Any]:
        if not prompt.strip():
            return {"status": "blocked", "reason": "empty_prompt"}
        started = time.monotonic(); digest = hashlib.sha256(prompt.encode()).hexdigest()
        try:
            import torch
            ...
            with torch.inference_mode():
                output = model.generate(**encoded, max_new_tokens=max(1, min(max_new_tokens, 256)), do_sample=False)
            text = tokenizer.decode(output[0][encoded["input_ids"].shape[1]:], skip_special_tokens=True)
            latency = round((time.monotonic() - started) * 1000, 2)
            result = {"status": "completed", "adapter_path": str(path), "response": text,
                      "latency_ms": latency, "canary": True, "production_share": 0.0}
            self._event(str(path), "canary_generation", True, latency, digest, result=result)
            return result
        except Exception as exc:
            self._event(adapter_path, "canary_generation", False, 0, digest, error=str(exc))
    ...
    def activate(self, adapter_path: str, *, approved_by: str) -> dict[str, Any]:
        ...
        path = self._resolve(adapter_path)
        with sqlite3.connect(self.db_path) as conn:
            passed = conn.execute("SELECT COUNT(*) FROM peft_canary_events WHERE adapter_path=? AND event='canary_generation' AND success=1", (str(path),)).fetchone()[0]
            if not passed:
                return {"status": "blocked", "reason": "successful_canary_required"}
            current = conn.execute("SELECT adapter_path FROM peft_serving_state WHERE slot='production'").fetchone()
            previous = current[0] if current else None
            conn.execute("""INSERT INTO peft_serving_state(slot,adapter_path,status,activated_at,approved_by,previous_adapter_path)
        ...
        return {"status": "active", "adapter_path": str(path), "previous_adapter_path": previous,
                "approved_by": approved_by, "rollback_ready": True}

    def rollback(self, *, approved_by: str) -> dict[str, Any]:
        if not approved_by:
            return {"status": "blocked", "reason": "named_human_approval_required"}
        with sqlite3.connect(self.db_path) as conn:
            row = conn.execute("SELECT adapter_path,previous_adapter_path FROM peft_serving_state WHERE slot='production'").fetchone()
            if not row:
                return {"status": "blocked", "reason": "no_active_adapter"}
            conn.execute("UPDATE peft_serving_state SET adapter_path=?,previous_adapter_path=?,status=?,approved_by=?,updated_at=CURRENT_TIMESTAMP WHERE slot='production'",
                         (row[1], row[0], "active" if row[1] else "base_only", approved_by))
        self.unload()
        return {"status": "rolled_back", "adapter_path": row[1], "removed_adapter": row[0]}
    ...
    def status(self) -> dict[str, Any]:
        with sqlite3.connect(self.db_path) as conn:
            ...
            row = conn.execute("SELECT * FROM peft_serving_state WHERE slot='production'").fetchone()
            recent = [dict(item) for item in conn.execute("SELECT * FROM peft_canary_events ORDER BY id DESC LIMIT 20")]
        return {"status": "ok", "serving": dict(row) if row else {"status": "base_only"},
                "loaded_in_process": self._loaded_adapter, "recent_events": recent}

    def _event(self, path: str, event: str, success: bool, latency: float, prompt_sha: str | None,
               *, result: dict[str, Any] | None = None, error: str | None = None, approved_by: str | None = None) -> None:
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("INSERT INTO peft_canary_events(adapter_path,event,approved_by,success,latency_ms,prompt_sha256,result_json,error) VALUES(?,?,?,?,?,?,?,?)",
                         (path, event, approved_by, int(success), latency, prompt_sha, json.dumps(result or {}), error))
