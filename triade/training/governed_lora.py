...
class GovernedLoraJobRunner:
    ...
    def run(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if not dataset.is_file() or not any(root == dataset.parent or root in dataset.parents for root in allowed):
        ...
        config = LoraTrainingConfig(base_model=str(payload.get("base_model") or "Qwen/Qwen2.5-0.5B-Instruct"),
                                    output_dir=str(output), max_steps=max_steps,
                                    maximum_gpu_minutes=min(float(payload.get("maximum_gpu_minutes") or 30), 120))
        with sqlite3.connect(self.db_path) as conn:
            job_id = conn.execute("INSERT INTO lora_jobs(goal_id,dataset_path,base_model,max_steps,status) VALUES(?,?,?,?,?)",
                                  (goal_id, str(dataset), config.base_model, max_steps, "running")).lastrowid
        try:
            result = RealLoraTrainer(config).train(dataset, ood_path=payload.get("ood_path"),
                                                   forgetting_path=payload.get("forgetting_path"), db_path=self.db_path)
            result.update({"job_id": job_id, "canary_required": True, "automatic_activation": False})
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("UPDATE lora_jobs SET status='trained_pending_canary',finished_at=CURRENT_TIMESTAMP,result_json=? WHERE id=?",
                             (json.dumps(result, default=str), job_id))
            return result
        except Exception as exc:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("UPDATE lora_jobs SET status='failed',finished_at=CURRENT_TIMESTAMP,error=? WHERE id=?", (str(exc), job_id))
            return {"status": "error", "job_id": job_id, "error": str(exc), "automatic_activation": False}
