...
class CognitiveIntegrator:
    ...
    def save_decision(self, decision: IntegratedDecision) -> None:
        ...
        self._conn.execute(
            ...
            (
                ...
                json.dumps(
                    {"trace": decision.trace, "notes": decision.notes}, default=str
                ),
                decision.created_at,
            ),
        )
        ...
        *,
        ...
        trace.append(
            TraceEntry(
                ...
                data={
                    "run_id": run_id,
                    "user_input": str(input_packet.user_input)[:200],
                },
            )
        )
        ...
        if qualia_packet:
            trace.append(
                TraceEntry(
                    step="4_qualia",
                    component="qualia",
                    action="experience_built",
                    result="ok",
                    timestamp=utc_now(),
                    data={
                        "meaning": qualia_packet.meaning.composite
                        if hasattr(qualia_packet, "meaning")
                        else 0,
                        "emotion": qualia_packet.emotion.label
                        if hasattr(qualia_packet, "emotion")
                        else "",
                    },
                )
            )
        ...
        trace.append(
            TraceEntry(
                ...
                data={
                    ...
                    "temporal": crystal.temporal_status,
                },
            )
        )

        self._detect_all_conflicts(
            signals, plan, crystal, memory, input_packet, resources, conflicts
        )
        ...
        trace.append(
            TraceEntry(
                ...
                result=f"{len(conflicts)} conflicts resolved",
                timestamp=utc_now(),
        ...
        trace.append(
            TraceEntry(
                ...
                action="full_check",
                result="all_passed" if all_passed else "violations_found",
                timestamp=utc_now(),
                data={"checks": checks},
            )
        )
        ...
        recommendation = self._emit_recommendation(
            all_passed, signals, crystal, conflicts
        )
        ...
        plan_adjusted = any(
            c.resolution
            for c in conflicts
            if c.field_name in {"steps", "risk", "urgency", "scope"}
        )
    ...
    def _detect_all_conflicts(
        ...
        urgency_val = {"low": 0.25, "medium": 0.55, "high": 0.85}.get(
            getattr(signals, "urgency", "medium"), 0.5
        )
        ...
        if any(
            kw in str(getattr(s, "description", "")).lower()
            for s in steps
            for kw in identity_kws
        ):
        ...
        if destructive_steps and not any(
            bool(getattr(s, "rollback_data", None)) for s in destructive_steps
        ):
        ...
        user_text = str(getattr(input_packet, "user_input", "")).lower()
        if any(kw in user_text for kw in privacy_kws) and not ctx.get(
            "user_consent", True
        ):
        ...
        if autonomy_flags:
            conflicts.append(
                ConflictRecord(
                    ...
                    value_a=autonomy_flags,
                    value_b="constitutional_constraints",
                    severity="high"
                    if "identity_mutation" in autonomy_flags
                    else "medium",
                )
            )

    def _resolve_conflict(
        self, conflict: ConflictRecord, signals: Any, plan: Any, crystal: Any
    ) -> None:
        strategy = CONFLICT_RESOLUTION_STRATEGIES.get(
            conflict.conflict_type, "safety_wins"
        )
        ...
        if conflict.conflict_type == "priority_vs_resources":
            ...
            if hasattr(signals, "notes"):
                signals.notes.append(
                    "Curiosity blocked by safety: risk elevated to high."
                )
    ...
    def _constitution_full_check(
        self, *, run_id: str, signals: Any, plan: Any, crystal: Any, input_packet: Any
    ) -> list[dict[str, Any]]:
        ...
        checks.append(
            self._check_article(
                "I",
                "Identidad Inmutable",
                not modifies_identity,
                "Identity safe"
                if not modifies_identity
                else "Identity mutation detected",
            )
        )
        ...
        checks.append(
            self._check_article(
                ...
                not is_critical or human_approved,
                "Approval ok"
                if (not is_critical or human_approved)
                else "Critical change without human approval",
            )
        )
        ...
        has_rollback = (
            any(bool(getattr(s, "rollback_data", None)) for s in steps)
            or hasattr(plan, "rollback")
            and plan.rollback is not None
        )
        checks.append(
            self._check_article(
                "III",
                "Rollback Obligatorio",
                not has_destructive or has_rollback,
                "Rollback present"
                if (not has_destructive or has_rollback)
                else "Destructive op without rollback",
            )
        )
        ...
        checks.append(
            self._check_article(
                "IV",
                "Aislamiento de Embeddings",
                not cross_domain,
                "Embedding isolation ok"
                if not cross_domain
                else "Cross-domain embedding leak detected",
            )
        )
        ...
        checks.append(
            self._check_article(
                ...
                f"Verification ok ({verification_steps} checks)"
                if (not high_risk or verification_steps >= 2)
        ...
        if hasattr(plan, "total_budget_cpu") and hasattr(plan, "total_budget_ram"):
            ...
            used_cpu = sum(
                getattr(getattr(s, "budget", None), "used_cpu_seconds", 0) or 0
                for s in steps
            )
            used_ram = sum(
                getattr(getattr(s, "budget", None), "used_ram_mb", 0) or 0
                for s in steps
            )
        ...
        checks.append(
            self._check_article(
                ...
                if graceful
                else f"Temporal status: {temporal}",
            )
        )
        ...
        checks.append(
            self._check_article(
                ...
                "Privacy ok"
                if (not privacy_risk or user_consent)
                else "Data privacy violation",
            )
        )
        ...
        checks.append(
            self._check_article(
                ...
                "Improvement safe"
                if safe_improvement
                else "Improvement blocked: system degraded",
            )
        )
    ...
    @staticmethod
    def _check_article(
        article_id: str, title: str, passed: bool, details: str
    ) -> dict[str, Any]:
    ...
    def _emit_recommendation(
        all_passed: bool, signals: Any, crystal: Any, conflicts: list[ConflictRecord]
    ) -> str:
        if not all_passed:
            return "block"
