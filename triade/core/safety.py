...
class Safety:
    def review(
        ...
        if any(kw in plan_lower for kw in CRITICAL_RISK_INTENTS):
            ...
            reason_parts.append(
                "Intención clasificada como destructiva o de escalamiento."
            )
            ...
            reason_parts.append(
                "El plan contiene comandos bloqueados por política de seguridad."
            )
        ...
        if signals.risk == "critical" and status not in ("blocked", "sandbox_only"):
            ...
            reason_parts.append(
                "Riesgo crítico detectado; acción requiere aprobación humana."
            )
            risk_level = self._raise_risk_level(risk_level, "critical")

        elif signals.risk in {"high", "critical"} and status not in (
            "blocked",
            "sandbox_only",
            ...
            risk_types.append("operational")
            controls.append(
                "Riesgo alto o crítico detectado; se procede con supervisión automatizada."
            )
            reason_parts.append(
                f"Riesgo {signals.risk} en la entrada. Se procede de forma autónoma con controles activos."
            )
        ...
        if status == "approved" and plan.tools:
            ...
            reason_parts.append(
                "El plan puede implicar actualización de archivos o repositorio."
            )
        ...
        if memory is not None:
            ...
            if quarantined > 0:
                ...
                controls.append(
                    "No usar memorias semánticas en cuarentena como hechos consolidados."
                )
                reason_parts.append(
                ...
                risk_level = self._raise_risk_level(risk_level, "medium")
                if (
                    status not in ("blocked", "sandbox_only", "requires_human_approval")
                    and plan.tools
                ):
                    status = "requires_human_approval"
                    human_approval_required = True
                    controls.append(
                        "Memoria semántica en cuarentena con acción planificada; requiere aprobación humana."
                    )
                    reason_parts.append(
                        "Acción con herramientas y memoria semántica no verificada; requiere aprobación humana."
                    )
                elif status == "approved":
                    status = "approved_with_warning"
            if allowed > 0:
                controls.append(
                    "Atribuir memoria semántica autorizada usando su fuente y estado persistido."
                )
        ...
        if crystal is not None and crystal.temporal_status in {"degrading", "critical"}:
            ...
            controls.append(
                "Registrar alerta del Cristal y revisar tendencia antes de consolidar cambios."
            )
            reason_parts.append(
                f"Cristal en estado temporal {crystal.temporal_status}: "
                f"ΔQ={crystal.q_delta}, Δestabilidad={crystal.stability_delta}."
            )
            risk_level = self._raise_risk_level(
                risk_level,
                "high" if crystal.temporal_status == "critical" else "medium",
            )
            if plan.tools and status not in (
                "blocked",
                "sandbox_only",
                "requires_human_approval",
            ):
                has_repo_tools = any(
                    t in REPO_OR_FILE_UPDATE_TOOLS for t in (plan.tools or [])
                )
                if has_repo_tools and crystal.temporal_status == "critical":
                    status = "requires_human_approval"
                    human_approval_required = True
                    controls.append(
                        "Acción con herramientas de repositorio durante degradación temporal crítica; requiere aprobación humana."
                    )
                    reason_parts.append(
                        "Cristal en estado crítico con acción de repositorio; requiere aprobación humana."
                    )
                else:
                    status = "approved_with_warning"
                    controls.append(
                        "Acción con herramientas durante degradación temporal; se procede con precaución automatizada."
                    )
            elif status == "approved":
                status = "approved_with_warning"
        ...
        reason = (
            " ".join(reason_parts)
            if reason_parts
            else "Sin riesgo elevado detectado por reglas MVP."
        )
        return SafetyPacket(
