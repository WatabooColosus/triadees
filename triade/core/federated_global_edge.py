...
def build_federated_global_edge_context(
    ...
    *,
    ...
    active = [
        n for n in sanitized if n["status"] == "active" and n["capabilities"]["online"]
    ]
    return {
        "status": "ok",
        ...
        "aggregate": {
            "cpu_count": sum(
                int(n["capabilities"].get("cpu_count") or 0) for n in active
            ),
            "ram_available_gb": round(
                sum(
                    float(n["capabilities"].get("ram_available_gb") or 0.0)
                    for n in active
                ),
                2,
            ),
            "llm_hosts": sum(
                1 for n in active if n["capabilities"].get("can_run_local_llm")
            ),
        },
        "policy": {
