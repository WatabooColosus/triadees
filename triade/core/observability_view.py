...
class TriadeObservabilityView:
    ...
    def build(self, limit: int = 20) -> dict[str, Any]:
        ...
        bodega = collect(
            "bodega",
            lambda: Bodega(db_path=self.db_path).doctor(runs_dir=self.runs_dir),
            self._empty_bodega(),
        )
        pulse = collect(
            "system_pulse",
            lambda: (
                self.system_pulse_fn(sync_relay=False) if self.system_pulse_fn else {}
            ),
            {},
        )
        health = collect("health", self.health_fn, {}) if self.health_fn else {}
        workers = collect(
            "workers", lambda: self._workers(limit), self._empty_workers()
        )
        learning = collect(
            "learning", lambda: self._learning(limit), self._empty_learning()
        )
        regression = collect(
            "regression_gate",
        ...
        neurons = collect(
            "neurons",
            lambda: NeuronIdentityView(self.db_path, self.runs_dir).list(limit=limit),
            self._empty_neurons(),
        )
        qualia = collect("qualia", lambda: self._qualia(limit), self._empty_qualia())
        federation = collect(
            "federation", lambda: self._federation(limit), self._empty_federation()
        )
        semantic = collect(
            "semantic_memory",
            lambda: SemanticMemoryStore(db_path=self.db_path).doctor(),
            {},
        )
        models = collect(
            "models", self._models, {"status": "unknown", "ollama": {"ok": False}}
        )
        recent_errors = collect(
            "internal_errors",
            lambda: query_internal_errors(limit=limit, db_path=self.db_path),
            [],
        )
        last_run = collect("last_run", self._last_run, None)
        memory_trace = collect(
            "memory_trace", lambda: self._last_run_memory_trace(), {}
        )
        ...
        if regression.get("status") == "attention":
            status = "degraded"
            warnings.append(
                "Regression Gate requiere atención: existen fallos, evidencia inválida o cuarentenas activas."
            )
        if capabilities.get("status") == "attention":
            status = "degraded"
            warnings.append(
                "Capability Registry requiere atención: existen capacidades bloqueadas."
            )
        ...
        if memory_trace and memory_trace.get("run_id"):
            last_run_data["memory_trace_summary"] = {
                ...
                "authorized_matches_count": memory_trace.get(
                    "authorized_matches_count", 0
                ),
                "quarantined_matches_count": memory_trace.get(
                    "quarantined_matches_count", 0
                ),
                "contradictions_count": memory_trace.get("contradictions_count", 0),
                "stable_needs_review": memory_trace.get("stable_needs_review", 0),
                "bodega_global_status": memory_trace.get(
                    "bodega_global_status", "unknown"
                ),
            }
        ...
        return {
            ...
            "internal_errors": {
                ...
                "message": None
                if recent_errors
                else "No hay errores internos recientes.",
            },
            "warnings": warnings,
            ...
            "empty_messages": {
                ...
                "regression_gate": "Regression Gate todavía no ha sido inicializado."
                if regression.get("status") == "not_initialized"
                else None,
                "capability_registry": "No hay capacidades registradas todavía."
                if capabilities.get("status") == "empty"
                else None,
            },
        }
    ...
    def _workers(self, limit: int) -> dict[str, Any]:
        service = WorkerBackgroundService(
            db_path=self.db_path, runs_dir=self.worker_runs_dir
        )
        status = service.status()
        events = service.events(limit=limit)
        queue = service.queue_status(limit=limit)
        last_error = next(
            (e for e in events.get("events", []) if e.get("status") == "error"), None
        )
        return {
            "status": "ok",
    ...
    def _learning(self, limit: int) -> dict[str, Any]:
        ...
        return {
            ...
            "message": None
            if pending
            else "No hay candidatos de aprendizaje pendientes.",
        }
        ...
        try:
            ...
            response = MemoryTraceResponse(
                ...
                memory_confidence_score=float(
                    trace.get("memory_confidence_score", 0.0)
                ),
                ...
                stable_needs_review=int(
                    (trace.get("stable_audit_summary") or {}).get(
    ...
    def _empty_learning() -> dict[str, Any]:
        return {
            ...
            "consolidated": 0,
            "rejected": 0,
            "message": "No hay candidatos de aprendizaje pendientes.",
        }
    ...
    def _empty_neurons() -> dict[str, Any]:
        return {
            ...
            "summary": {"total_neurons": 0, "by_status": {}},
            "neurons": [],
        }
