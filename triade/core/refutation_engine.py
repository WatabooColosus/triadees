...
def _claim(
    claim_id: str, statement: str, check: Callable[[], tuple[bool | None, Any]]
) -> dict[str, Any]:
    try:
...
def run_system_refutation(
    ...
    def runtime_claim():
        ...
        return bool(observed_cycles > 0), {
            ...
            "background_thread_alive": value.get("background_thread_alive"),
            ...
            "continuity": value.get("runtime_continuity_score"),
    ...
    def edge_claim():
        ...
        ok = (
            value.get("status") == "ok"
            and "nodes" in value
            and value.get("policy", {}).get("node_input_is_evidence_not_truth") is True
        )
        return ok, {
            "nodes_total": value.get("nodes_total"),
            "nodes_active_online": value.get("nodes_active_online"),
    ...
    def artifacts_claim():
        ...
        return (bool(closed) and len(closed) == len(integrity)), {
            "closed_runs": len(closed),
            "integrity_artifacts": len(integrity),
        }
    ...
    def model_claim():
        ...
        return present, {
            "models": models,
            "truth": "Modelo derivado; no entrenamiento fundacional desde cero.",
        }

    claims = [
        _claim(
            "runtime_continuous",
            "El runtime opera continuamente en segundo plano.",
            runtime_claim,
        ),
        _claim(
            "foundational_neurons",
            "Las diez neuronas fundacionales están registradas.",
            foundational_claim,
        ),
        _claim(
            "global_edge",
            "Bodega Global incluye contexto de nodos federados con procedencia.",
            edge_claim,
        ),
        _claim(
            "run_integrity",
            "Los runs cerrados tienen integridad auditable.",
            artifacts_claim,
        ),
        _claim(
            "triade_model",
            "Existe un modelo Ollama identificable de Tríade.",
            model_claim,
        ),
    ]
    counts = {
        verdict: sum(1 for item in claims if item["verdict"] == verdict)
        for verdict in ("supported", "falsified", "undetermined")
    }
    return {
