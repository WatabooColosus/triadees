...
def candidates_from_system_debt(
    ...
    if (
        pulse_status in {"degraded", "critical", "warning", "error", "unknown"}
        or "degrad" in pulse_summary_text.lower()
    ):
        add(
            _candidate(
    ...
    federation_declared = isinstance(pulse.get("federation"), dict) and bool(
        pulse.get("federation")
    )
