...
def _seed_missions(store: NeuronMissionStore) -> list[int]:
    ...
    for title, mission, domain, status in [
        ...
        (
            "Cooking Bot",
            "Generate cooking recipes from ingredients",
            "cooking",
            "candidate",
        ),
        ...
        m = NeuronMission(
            neuron_id=1, title=title, mission=mission, domain=domain, status=status
        )
...
def test_select_filters_by_domain():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        result = select_relevant_missions(
            domain="ml", db_path=Path(tmp) / "test.db", limit=10
        )
...
def test_select_rejects_paused():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        {m["id"] for m in result["selected"]} | rejected_ids
...
def test_limit_respected():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        _seed_missions(store)
        result = select_relevant_missions(
            domain="ml", db_path=Path(tmp) / "test.db", limit=1
        )
        assert result["count"] <= 1
...
def test_empty_db():
    with tempfile.TemporaryDirectory() as tmp:
        result = select_relevant_missions(
            user_input="test", db_path=Path(tmp) / "test.db"
        )
        assert result["status"] == "ok"
