...
def _docs_actuales() -> list[Path]:
    ...
    rutas += [
        p for p in (REPO_ROOT / "docs").rglob("*.md") if "audits" not in p.parts
    ]
    return sorted(
...
def test_no_se_documentan_modulos_retirados() -> None:
    ...
    quedó atrás.
    """
    retirados = ("triade/core/plan_step.py", "triade/core/hierarchical_pulse.py")
    vivos: list[str] = []
    for doc in _docs_actuales():
        texto = doc.read_text(encoding="utf-8")
        for modulo in retirados:
            if modulo in texto and not (REPO_ROOT / modulo).exists():
                # Mencionarlo explicando que se retiró es correcto; presentarlo
                # como existente, no. La marca es que el propio texto lo diga.
                if not re.search(r"retirad|archivad|elimina", texto, re.IGNORECASE):
