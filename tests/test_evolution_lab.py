...
def make_campaign(lab, policy=None):
    return lab.create_campaign(
        "Mejora A", "A mejora generalización", "v1", "v2", policy
    )


def pass_measurement(lab, campaign_id):
    cases = [
        {"id": f"case-{i}", "domain": domain, "prompt": f"sealed-{i}"}
...
def test_measurement_blocks_regression(tmp_path):
    ...
    lab.record_evidence(
        ...
        "baseline_comparison",
        {
            ...
            "candidate_scores": {d: 0.8 for d in DOMAINS},
            "regressions": {"safety": -0.08},
        },
        source="runner",
        independent=True,
    )
...
def test_full_six_stage_campaign_requires_evidence_and_is_signed(tmp_path):
    ...
    for kind, payload in {
        ...
        lab.record_evidence(
            ...
            payload,
            source="adapter-lab",
            independent=kind != "adapter_training",
        )
    ...
    for evaluator in ("external-a", "external-b"):
        lab.record_evidence(
            ...
            {
                "evaluator": evaluator,
                "suite": "hidden-generalization",
                "score": 0.79,
                "report_hash": f"hash-{evaluator}",
                "signature": f"sig-{evaluator}",
            },
            source=evaluator,
            independent=True,
        )
...
def test_research_cannot_write_stable_memory(tmp_path):
    ...
    lab.record_evidence(
        cid, Stage.EXPERIENCE, "replication", lesson, source="b", independent=True
    )
    lab.advance(cid)
    ...
    lab.record_evidence(
        ...
        Stage.ADAPTER,
        "canary",
        {"observations": 3, "rollback_ready": True},
        source="lab",
    )
    lab.advance(cid)
    payload = {
    ...
    payload.update(
        {
            "sources": [{"url": "https://example.org", "retrieved_at": "now"}],
