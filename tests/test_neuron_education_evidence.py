...
def prepared_session(tmp_path, *, candidate_id="candidate-source-a"):
    ...
    competency = store.ensure_competency(
        int(neuron_id), "research", "contrastar fuentes"
    )
    curriculum = store.ensure_curriculum(
        int(neuron_id), None, "research", "contrastar fuentes"
    )
    session = store.record_session(
        ...
        material_refs=["https://a.example/doc", "https://b.example/doc"],
        independent_sources=2,
        lesson={"truth_status": "candidate", "candidate_ids": [candidate_id]},
        exercise={"type": "held_out"},
...
def test_learning_requires_independent_improvement_and_three_unique_runs(tmp_path):
    ...
    evaluated = store.record_independent_evaluation(
        session_id,
        evaluator_id="critical_evaluator:v1",
        baseline_score=0.4,
        post_score=0.8,
    ...
    first = store.record_run_application(
        session_id, run_id="run-1", outcome_score=0.8, evidence_ref="run-evaluation:1"
    )
    duplicate = store.record_run_application(
        session_id,
        run_id="run-1",
        outcome_score=1.0,
        evidence_ref="run-evaluation:duplicate",
    )
    assert first["learned"] is False
    assert duplicate["duplicate"] is True
    assert duplicate["applied_run_count"] == 1
    store.record_run_application(
        session_id, run_id="run-2", outcome_score=0.75, evidence_ref="run-evaluation:2"
    )
    final = store.record_run_application(
        session_id, run_id="run-3", outcome_score=0.9, evidence_ref="run-evaluation:3"
    )
