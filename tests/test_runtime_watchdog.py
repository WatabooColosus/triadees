...
def _healthy_db(path):
    ...
    with sqlite3.connect(path) as conn:
        ...
        """)
        conn.execute("INSERT INTO worker_state VALUES('workers',?, '{}')", (datetime.now(timezone.utc).isoformat(),))


def test_health_detects_stopped_process(tmp_path, monkeypatch):
    path = tmp_path / "health.db"
    _healthy_db(path)
    monkeypatch.setattr("triade.runtime.service_health.build_resource_probe", lambda: {
        "disk": {"free_gb": 10}, "memory": {"available_gb": 10}, "thermal": {"thermal_status": "ok"}
    })
    status = ServiceHealth(path).inspect(process_running=False, ollama_probe={"ok": True})
...
def test_health_detects_frozen_heartbeat(tmp_path, monkeypatch):
    ...
    status = ServiceHealth(path, heartbeat_stale_seconds=10).inspect(process_running=True, ollama_probe={"ok": True})
...
def test_recovery_snapshots_recovers_lease_and_checks_heartbeat(tmp_path):
    ...
    store.claim("dead")
    with sqlite3.connect(path) as conn:
        conn.execute("UPDATE autonomous_tasks SET lease_expires_at='2000-01-01T00:00:00+00:00' WHERE task_id=?", (task["task_id"],))
    recovery = RuntimeRecovery(path, tmp_path / "snapshots").recover("test", verify_heartbeat=lambda: True)
...
def test_watchdog_honors_recovery_budget(tmp_path, monkeypatch):
    ...
    first = watchdog.tick(process_running=False, ollama_probe={"ok": True}, verify_heartbeat=lambda: True)
    second = watchdog.tick(process_running=False, ollama_probe={"ok": True}, verify_heartbeat=lambda: True)
