...
class TestAlwaysOnConfig:
    ...
    def test_env_override_mode(self):
        ...
        with patch.dict(
            os.environ,
            {"TRIADE_ALWAYS_ON": "true", "TRIADE_ALWAYS_ON_MODE": "observe_only"},
            clear=True,
        ):
            cfg = load_always_on_config()
        assert cfg.get("mode") == "observe_only"
    ...
    def test_env_override_interval(self):
        ...
        with patch.dict(
            os.environ, {"TRIADE_ALWAYS_ON_INTERVAL_SECONDS": "120"}, clear=True
        ):
            cfg = load_always_on_config()
...
class TestAlwaysOnStartup:
    ...
    def test_always_on_true_in_yml_starts_on_startup(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ao, "start_internal_runtime_background", lambda **_kw: {"status": "started"}
        )
        monkeypatch.setattr(ao, "run_self_test_cycle", lambda **_kw: {"status": "ok"})
        monkeypatch.setattr(
            ao, "record_internal_runtime_event", lambda *_args, **_kw: None
        )
...
class TestSelfTestCycle:
    def test_self_test_safe_mode(self, tmp_path):
        ...
        result = run_self_test_cycle(
            mode="safe", db_path=db, runs_dir=tmp_path / "runs"
        )
