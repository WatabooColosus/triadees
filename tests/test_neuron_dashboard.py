...
def test_neuron_dashboard_is_read_only_and_groups_status(tmp_path: Path) -> None:
    ...
    dashboard = build_neuron_dashboard(
    ...
    assert (
        dashboard["policy"] == "dashboard_read_only_actions_require_explicit_endpoint"
    )
    ...
    candidate = next(
        n for n in dashboard["neurons"] if n["name"] == "neurona-dashboard-candidate"
    )
    stable = next(
        n for n in dashboard["neurons"] if n["name"] == "neurona-dashboard-stable"
    )

    assert any(
        a["id"] == "approve_experimental" and a["enabled"]
        for a in candidate["ui_actions"]
    )
