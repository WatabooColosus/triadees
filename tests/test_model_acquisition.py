...
def test_catalog_has_provenance_license_and_budget():
    ...
    assert all(
        item["source"].startswith("https://ollama.com/library/")
        for item in MODEL_CATALOG.values()
    )
    assert all(
        item["license"] and item["size_gb"] > 0 for item in MODEL_CATALOG.values()
    )
...
def test_reconcile_never_downloads_outside_catalog(tmp_path, monkeypatch):
    with (
        patch(
            "triade.core.model_acquisition.OllamaClient.health",
            return_value={"models": list(MODEL_CATALOG)},
        ),
...
def test_failed_pull_closes_the_original_attempt(tmp_path):
    db = tmp_path / "triade.db"
    installed = list(MODEL_CATALOG)[1:]
    with (
        patch(
            "triade.core.model_acquisition.OllamaClient.health",
            return_value={"models": installed},
        ),
        patch(
            "triade.core.model_acquisition._ollama_binary",
            return_value="/usr/bin/ollama",
        ),
        patch(
            "triade.core.model_acquisition.subprocess.run",
            side_effect=subprocess.TimeoutExpired(["ollama", "pull"], 1800),
        ),
    ):
        result = reconcile_model_catalog(
            db_path=db, max_downloads=1, min_free_disk_gb=0
        )

    with sqlite3.connect(db) as conn:
        rows = conn.execute(
            "SELECT status, finished_at FROM model_acquisition_attempts"
        ).fetchall()
...
def test_assignments_preserve_innate_default(tmp_path):
    ...
    with patch(
        "triade.core.model_acquisition.OllamaClient.health",
        return_value={"models": ["triade-omega:latest"]},
    ):
...
def test_specialized_neurons_receive_specialized_models(tmp_path):
    ...
    registry.register(
        NeuronSpec(
            name="Neurona de imágenes",
            mission="Analizar imágenes",
            domain="vision",
            status="experimental",
        )
    )
    registry.register(
        NeuronSpec(
            name="Neurona reparadora",
            mission="Reparar código",
            domain="code_repair",
            status="experimental",
        )
    )
    ...
    with patch(
        ...
        result = {
            item["neuron"]: item["model"] for item in assign_models_to_neurons(db)
        }
