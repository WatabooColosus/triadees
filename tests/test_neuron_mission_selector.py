...
def test_select_relevant_missions_domain_match(tmp_path: Path) -> None:
    ...
    result = select_relevant_missions(
        user_input="estado interno", domain="memory", db_path=db_path, limit=5
    )
    assert result["selected"]
    assert result["selected"][0]["domain"] == "memory"
    assert (
        result["selected"][0]["relevance_score"]
        >= result["selected"][-1]["relevance_score"]
    )
...
def test_select_relevant_missions_keyword_match(tmp_path: Path) -> None:
    ...
    assert result["selected"][0]["title"] == "keyword-mission" or result["selected"][0][
        "mission"
    ].startswith("Auditar memoria")
    assert result["selected"][0]["reason"]
    assert (
        "keyword_match" in result["selected"][0]["reason"]
        or "domain_match" in result["selected"][0]["reason"]
    )
...
def test_selector_does_not_modify_identity_core(tmp_path: Path) -> None:
    ...
    result = select_relevant_missions(
        user_input="estado", domain="system", db_path=db_path, limit=5
    )
...
def test_relevant_missions_endpoint_and_alias_are_read_only(
    ...
    monkeypatch.setattr(
        ...
        fake_select_relevant_missions,
    )

    client = TestClient(app)
    resp = client.get(
        "/api/neurons/missions/relevant",
        params={"query": "memoria", "domain": "observability"},
    )
    alias = client.get(
        "/api/system/neurons/missions/relevant",
        params={"query": "memoria", "domain": "observability"},
    )
...
def test_run_orchestrator_uses_relevant_missions(tmp_path: Path, monkeypatch) -> None:
    ...
    mission_id = mission_store.get_missions_by_neuron(
        int(registry.get_neuron("orchestrator-mission")["id"])
    )[0].id
    ...
    input_packet = InputPacket(
        ...
        source="tests",
        context={"semantic_domain": "observability"},
    )
    ...
    assert (
        output.memory_diff["mission_selection_policy"]["selector_is_read_only"] is True
    )
...
def test_worker_loop_blocks_irrelevant_mission(tmp_path: Path, monkeypatch) -> None:
    db_path = make_db(tmp_path)
    neuron = register_neuron(
        db_path,
        name="worker-skip-neuron",
        status="experimental",
        domain="observability",
    )
    NeuronMissionStore(db_path=db_path)
    mission_id = backfill_neuron_missions(
        db_path=db_path, runs_dir=tmp_path / "runs", limit=10
    )["created"][0]["id"]
    ...
    def _boom(*args, **kwargs):
        raise AssertionError(
            "NeuronMissionExecutor no debe ejecutarse si la misión es irrelevante"
        )

    monkeypatch.setattr(
        "triade.workers.neuron_mission_executor.NeuronMissionExecutor.execute", _boom
    )
