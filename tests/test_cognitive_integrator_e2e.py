...
def _make_signals(
    *, risk: str = "low", urgency: str = "low", intent: str = "conversation"
) -> SimpleNamespace:
    return SimpleNamespace(
        ...
        notes=[],
        pv7={},
        to_dict=lambda: {"risk": risk, "urgency": urgency},
    )
...
def _make_crystal(
    *, q_crystal: float = 0.6, temporal_status: str = "baseline"
) -> SimpleNamespace:
...
class TestCognitiveIntegratorE2E:
    ...
    def test_identity_mutation_detected(self) -> None:
        ...
        article_i = next(
            c for c in decision.constitution_checks if c["article_id"] == "I"
        )
    ...
    def test_destructive_step_without_rollback_blocks(self) -> None:
        ...
        plan = _make_plan(
            [
                PlanStep(
                    id="s1",
                    description="destructive data wipe",
                    step_type="destructive",
                ),
            ]
        )
        decision = integrator.integrate(
        ...
        article_iii = next(
            c for c in decision.constitution_checks if c["article_id"] == "III"
        )
    ...
    def test_degraded_crystal_triggers_blocks(self) -> None:
        ...
        article_viii = next(
            c for c in decision.constitution_checks if c["article_id"] == "VIII"
        )
    ...
    def test_autonomy_vs_constitution_conflict_detected(self) -> None:
        ...
        plan = _make_plan(
            [
                PlanStep(
                    id="s1", description="Modify identity_core", step_type="action"
                ),
            ]
        )
        decision = integrator.integrate(
            run_id="e2e-auto",
            input_packet=_make_input(
                "aprende y modifica identidad", human_approved=True
            ),
            signals=_make_signals(risk="high"),
            memory=_make_memory(),
            crystal=_make_crystal(),
