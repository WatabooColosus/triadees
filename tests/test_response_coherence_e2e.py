...
def test_runner_factual_simple_does_not_create_neuron(
    ...
    _wire_runner(
        monkeypatch,
        runner,
        response="Colombia se encuentra en el continente de América del Sur.",
    )
    ...
    assert (
        result["response"]
        == "Colombia se encuentra en el continente de América del Sur."
    )
    assert result["neuron_proposal"] is None
    assert result["neuron_candidate_gate"]["route"] == "learning_candidate"
    assert (
        result["response_coherence_gate"]["detected_input_type"] == "factual_question"
    )


def test_runner_feedback_rewrites_without_repeating_previous_answer(
    tmp_path: Path, monkeypatch
) -> None:
    runner = make_runner(tmp_path)
    _wire_runner(
        monkeypatch,
        runner,
        response="Entendido. Colombia se encuentra en el continente de América del Sur.",
    )
    ...
    result = runner.run(
        ...
        context={
            "conversation_history": [
                ...
                {
                    "role": "bot",
                    "content": "Colombia se encuentra en el continente de América del Sur.",
                },
            ]
        },
    )
    ...
    assert (
        result["response_coherence_gate"]["detected_input_type"] == "positive_feedback"
    )
    assert result["memory_diff"]["traceability"]["response_coherence_gate_status"] in {
        "rewritten",
        "blocked",
    }


def test_runner_explicit_neuron_request_creates_candidate(
    tmp_path: Path, monkeypatch
) -> None:
    runner = make_runner(tmp_path)
    _wire_runner(
        monkeypatch,
        runner,
        response="Entendido. Crearé una neurona para eso.",
        intent="build_or_update",
    )

    result = runner.run(
        "crea una neurona para auditar memoria y evitar contradicciones"
    )
