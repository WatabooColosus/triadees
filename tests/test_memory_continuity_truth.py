...
def test_false_ephemeral_claim_is_replaced(tmp_path):
    ...
    response, corrections = enforce_memory_truth(
        "¿Recuerdas fuera de cada sesión?", "No guardo nada; todo el contenido desaparecerá.", snapshot
    )
...
def test_truthful_answer_is_unchanged(tmp_path):
    ...
    assert enforce_memory_truth("¿Tienes memoria?", original, memory_truth_snapshot(db)) == (original, [])
...
def test_vague_identity_answer_is_replaced_with_direct_continuity(tmp_path):
    ...
    db.touch()
    response, corrections = enforce_memory_truth(
        "¿Recuerdas fuera de cada sesión?", "Soy Tríade y tengo una Bodega.", memory_truth_snapshot(db)
    )
