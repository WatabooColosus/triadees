...
def test_runtime_api_endpoints(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        api_module, "get_internal_runtime_supervisor", lambda **_: supervisor
    )
    monkeypatch.setattr(
        api_module, "get_internal_runtime_state", lambda **_: supervisor.snapshot()
    )
    monkeypatch.setattr(
        api_module,
        "build_internal_context_snapshot",
        lambda **kwargs: {"status": "ok", "runtime": supervisor.snapshot()},
    )
    monkeypatch.setattr(
        api_module,
        "start_internal_runtime_background",
        lambda **kwargs: {"status": "started", "snapshot": supervisor.snapshot()},
    )
    monkeypatch.setattr(
        api_module,
    ...
    monkeypatch.setattr(
        ...
        lambda user_input="", limit=10: build_living_context_for_chat(
            user_input, db_path=db_path, runs_dir=runs_dir, limit=limit
        ),
    )
    monkeypatch.setattr(
        api_module,
        "build_living_report",
        lambda limit=20: build_living_report(
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: {
            ...
            "neurons": [],
            "policy": {"read_only_by_default": True},
        },
    )
    monkeypatch.setattr(
        api_module,
    ...
    assert (
        client.post("/api/runtime/once", json={"mode": "observe_only"}).status_code
        == 200
    )
    ...
    assert (
        client.get(
            "/api/system/living-context", params={"user_input": "estado runtime"}
        ).status_code
        == 200
    )
    ...
    assert (
        client.get("/api/bodega/global-context", params={"query": "test"}).status_code
        == 200
    )
    assert (
        client.get(
            "/api/system/bodega/global-context", params={"query": "test"}
        ).status_code
        == 200
    )
