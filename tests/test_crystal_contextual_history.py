...
def test_bodega_filters_crystal_history_by_context_key(tmp_path) -> None:
    ...
    signals_a = SignalPacket(
        run_id="run-a", intent="conversation", tone="neutral", urgency="low", risk="low"
    )
    signals_b = SignalPacket(
        run_id="run-b", intent="conversation", tone="neutral", urgency="low", risk="low"
    )
    basis_a = {
        "context_scope": "project",
        "context_key": "project|intent=conversation|project_id=a",
        "source": "test",
        "intent": "conversation",
        "project_id": "a",
    }
    basis_b = {
        "context_scope": "project",
        "context_key": "project|intent=conversation|project_id=b",
        "source": "test",
        "intent": "conversation",
        "project_id": "b",
    }
    ...
    crystal_a = Crystal().regulate(
        signals_a,
        MemoryPacket(run_id="run-a", confidence=0.8),
        comparison_basis=basis_a,
    )
    bodega.store_crystal(crystal_a)
    bodega.create_run(InputPacket(user_input="B", source="test", run_id="run-b"))
    crystal_b = Crystal().regulate(
        signals_b,
        MemoryPacket(run_id="run-b", confidence=0.8),
        comparison_basis=basis_b,
    )
...
def test_runner_starts_baseline_for_new_project_and_continues_same_project(
    tmp_path,
) -> None:
    runner = TriadeRunner(
    ...
    assert (
        first_a["crystal_temporal_state"]["context_key"]
        != first_b["crystal_temporal_state"]["context_key"]
    )

    payload_b = json.loads(
        (tmp_path / "runs" / first_b["run_id"] / "crystal.json").read_text(
            encoding="utf-8"
        )
    )
