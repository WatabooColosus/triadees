    ...
    class SkippedService:
        def context_probe(self, text):
            return EdgeProcessingResult(
                ...
                "no_node",
                "",
                {"status": "skipped", "reason": "no_node"},
            )

    monkeypatch.setattr(
        "triade.core.edge_context.EdgeProcessingService", SkippedService
    )
    monkeypatch.setattr(
...
def test_latest_local_signal_marks_historical_failures_recovered(tmp_path):
    from triade.core.edge_observations import (
        build_edge_context_health,
    ...
    for _ in range(3):
        record_edge_observation(
            "intent_probe", "empty_response", "empty", True, "", "hola", db_path=db
        )
    record_edge_observation(
        "local_edge_signal", "local_heuristic", "medium", False, "", "hola", db_path=db
    )
