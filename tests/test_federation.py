...
def test_register_node_rejects_forbidden_permissions(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="prohibidos"):
        federation.register_node(
            "n1", "Nodo", permissions=["receive_knowledge", "modify_identity_core"]
        )
...
def test_receive_requires_active_node_and_permission(tmp_path: Path) -> None:
    ...
    # nodo desconocido
    assert (
        federation.receive_exchange("ghost", "knowledge", "hola")["decision"]
        == "blocked"
    )
...
def test_received_knowledge_enters_as_candidate_never_consolidated(
    tmp_path: Path,
) -> None:
    federation = fed(tmp_path)
    federation.register_node(
        "n1", "Aliado", trust_level="high", permissions=["receive_knowledge"]
    )
...
def test_critical_risk_exchange_is_blocked(tmp_path: Path) -> None:
    ...
    result = federation.receive_exchange(
        "n1", "knowledge", "acción peligrosa", risk_level="critical"
    )
...
def test_send_blocks_sensitive_data_leak(tmp_path: Path) -> None:
    ...
    leaked = federation.send_exchange(
        "n1", "knowledge", "aquí está el api_key secreto: 1234"
    )
    assert leaked["decision"] == "blocked"

    ok = federation.send_exchange(
        "n1", "knowledge", "resumen público de un patrón de marketing"
    )
...
def test_doctor_reports_policy_and_counts(tmp_path: Path) -> None:
    federation = fed(tmp_path)
    federation.register_node(
        "n1", "Aliado", trust_level="medium", permissions=["receive_knowledge"]
    )
    federation.receive_exchange(
        "n1", "knowledge", "dato útil con fuente", domain="general"
    )
...
def test_federation_tracks_capable_compute_nodes(tmp_path: Path) -> None:
    ...
    assert (
        federation.list_capable_nodes(min_tier="medium", require_gpu=True)[0]["node_id"]
        == "gpu"
    )
