        ...
        for index in range(3):
            ...
            conn.execute(
                ...
                (
                    ...
                    "analiza memoria cristal fallback nucleo"
                    if index < 2
                    else "hola continuidad conversacional",
                    "ok",
                    "qwen2.5:3b-instruct",
                    "qwen2.5:3b-instruct",
                    f"2026-06-0{index + 1}",
            ...
            conn.execute(
                ...
                (
                    run_id,
                    "analyze" if index < 2 else "conversation",
                    "constructive",
                    "medium",
                    "low",
                    "{}",
            ...
            conn.execute(
                ...
                (
                    ...
                    0.04 * index,
                    0.02 * index,
                    "improving" if index == 2 else "stable",
                ),
            )
            conn.execute(
                ...
                (
                    ...
                    0.8,
                    0.85,
                    '["fallback observado"]' if index == 1 else "[]",
                    "[]",
                ),
            )
            conn.execute(
            ...
            conn.execute(
                ...
                (
                    run_id,
                    "central",
                    "template" if index == 1 else "ollama",
                    "template-fallback" if index == 1 else "qwen2.5:3b-instruct",
                    0 if index == 1 else 1,
                    "boom" if index == 1 else None,
                    0.6,
                ),
            )
            conn.execute(
...
def test_conversation_analyzer_exports_markdown_without_mutating_identity(
    tmp_path: Path,
) -> None:
    db_path = make_db(tmp_path)
