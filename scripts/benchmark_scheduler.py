...
def run(logical_hours: int) -> dict[str, float | int]:
    ...
    scheduler.add_job("heartbeat", lambda: counts.__setitem__("heartbeat", counts["heartbeat"] + 1), interval_seconds=5, run_immediately=True)
    scheduler.add_job("dispatch", lambda: counts.__setitem__("dispatch", counts["dispatch"] + 1), interval_seconds=20, run_immediately=True)
    started = time.perf_counter()
    for second in range(logical_hours * 3600 + 1):
        clock.value = float(second)
        scheduler.execute_due()
    elapsed = time.perf_counter() - started
    return {"logical_hours": logical_hours, **counts, "wall_seconds": elapsed, "job_count": scheduler.snapshot()["job_count"]}
