...
def main() -> int:
    ...
    parser.add_argument("--prompt", default="Instrucción: ¿Recuerdas después de cerrar una sesión?\nRespuesta:")
    parser.add_argument("--max-new-tokens", type=int, default=48)
    args = parser.parse_args()
    adapter = Path(args.adapter).resolve()
    manifest = json.loads((adapter / "triade_adapter_manifest.json").read_text(encoding="utf-8"))
    ...
    print(json.dumps({"loaded": True, "base_model": base, "adapter": str(adapter), "response": text}, ensure_ascii=False, indent=2))
    return 0
