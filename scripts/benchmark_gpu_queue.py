...
def snapshot() -> dict[str, object]:
    ...
    return {
        "status": "ok", "gpu": values[0], "vram_total_mb": float(values[1]),
        "vram_used_mb": float(values[2]), "vram_free_mb": float(values[3]),
        "temperature_c": float(values[4]), "driver": values[5],
