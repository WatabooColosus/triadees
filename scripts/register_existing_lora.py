...
def main():
    g = GovernedDatasets()
    source = Path("data/lora/triade_continuity_smoke.jsonl")
    existing = next((d for d in g.list_datasets() if d.source == str(source)), None)
    if existing:
        ds = existing
    else:
        ds = g.create_dataset(
            "Triade continuity governed",
            "Dataset smoke de continuidad",
            "memory_continuity",
            str(source),
            {
    ...
    for manifest_path in Path("artifacts/adapters").glob(
        "*/triade_adapter_manifest.json"
    ):
        name = manifest_path.parent.name
        manifest = json.loads(manifest_path.read_text())
        if name in known:
            registered.append(known[name].adapter_id)
            continue
        adapter = g.create_adapter(
            name, manifest["base_model"], ds.dataset_id, manifest.get("config", {})
        )
        metrics = {
        ...
        g.update_adapter_status(adapter.adapter_id, "evaluated", metrics)
