...
def main() -> int:
    ...
    parser.add_argument(
        "--apply",
        action="store_true",
        help="copiar unidades; no habilita ni inicia servicios",
    )
    parser.add_argument("--target", default="/etc/systemd/system")
    args = parser.parse_args()
    source = Path(__file__).resolve().parents[1] / "deploy/systemd"
    target = Path(args.target).resolve()
    units = sorted(source.glob("triade-*.service")) + sorted(
        source.glob("triade-*.timer")
    )
    for unit in units:
    ...
    if not args.apply:
        print(
            "dry-run: use --apply explicitly; daemon-reload/enable/start remain manual"
        )
    return 0
